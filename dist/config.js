...
function victusComUrl(url) {
    return url.replace(/victuscloud\.xyz/gi, 'victuscloud.com');
}
function splitApiKeys(...names) {
    const keys = names.flatMap((name) => (process.env[name] || '').split(/[\r\n,]+/));
    return [...new Set(keys.map((key) => key.trim()).filter(Boolean))];
}
function normalizeGroqUrl(raw) {
    const val = (raw || '').trim();
    if (!val || /cognitiveservices\.azure\.com/i.test(val))
        return 'https://api.groq.com/openai/v1';
    let url = val.replace(/\/+$/, '');
    if (url.endsWith('/openai'))
        url += '/v1';
    else if (!url.endsWith('/v1') && !url.includes('/v1/'))
        url += '/v1';
    return url;
}
