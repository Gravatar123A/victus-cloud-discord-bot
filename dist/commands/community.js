import { SlashCommandBuilder, MessageFlags, ActionRowBuilder, ButtonBuilder, ButtonStyle, MediaGalleryBuilder, MediaGalleryItemBuilder, } from 'discord.js';
...
function renderProgressBar(current, needed, size = 14) {
    const ratio = Math.max(0, Math.min(1, current / (needed || 1)));
    const filled = Math.round(ratio * size);
    const empty = size - filled;
    return `\`[${'█'.repeat(filled)}${'░'.repeat(empty)}] ${Math.round(ratio * 100)}%\``;
}
export const battlepassCommand = {
    ...
    async execute(interaction) {
        ...
        if (bpCardAttachment) {
            container.addMediaGalleryComponents(new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(`attachment://${bpCardAttachment.name}`)));
        }
...
export const smpNetworkCommand = {
    data: new SlashCommandBuilder()
        .setName('smp-network')
        ...
        .addStringOption((opt) => opt.setName('ip').setDescription('Server IP address or domain').setRequired(true))
        .addIntegerOption((opt) => opt.setName('port').setDescription('Port (default 25565)').setRequired(false))
        .addStringOption((opt) => opt.setName('category').setDescription('Category (Survival, Lifesteal, Skyblock, RPG)').setRequired(false))
        .addStringOption((opt) => opt.setName('description').setDescription('Short description of your server').setRequired(false))),
    cooldown: 4,
    async execute(interaction) {
        ...
        if (sub === 'list') {
            ...
            const container = ComponentsV2.baseContainer(ComponentsV2.Accents.primary);
            if (servers.length === 0) {
                container.addTextDisplayComponents(ComponentsV2.text(`# 🌐 Victus SMP Network Directory\n\n` +
...
export const warCommand = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((opt) => opt.setName('target-guild-id').setDescription('Target Server Guild ID').setRequired(true)))
        .addSubcommand((sub) => sub
        .setName('status')
    ...
    async execute(interaction) {
        ...
        if (sub === 'status') {
            ...
            const container = ComponentsV2.baseContainer(myPoints >= enemyPoints ? ComponentsV2.Accents.success : ComponentsV2.Accents.danger);
