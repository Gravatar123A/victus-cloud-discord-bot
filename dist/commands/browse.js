...
export function createBrowserSession(userId, initial = {}) {
...
export function getBrowserSession(token) {
...
export const browseCommand = {
    data: new SlashCommandBuilder()
        ...
        .addBooleanOption((opt) => opt.setName('ephemeral').setDescription('Make the response visible only to you (default: false)')),
    cooldown: 3,
    ...
    async handleButton(interaction) {
        ...
        // Ensure page bound
        if (session.page > data.totalPages)
            session.page = data.totalPages;
        const { container, actionRows } = DiscoveryEmbeds.buildBrowser(data, session);
    ...
    async handleModal(interaction) {
        if (!interaction.customId.startsWith('browse_modal:'))
            return;
        const sessionToken = interaction.customId.split(':')[1];
        const session = getBrowserSession(sessionToken);
        if (!session) {
