import { MessageFlags, PermissionFlagsBits, SlashCommandBuilder, ActionRowBuilder, StringSelectMenuBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, ButtonBuilder, ButtonStyle } from 'discord.js';
import { ComponentsV2 } from '../embeds/componentsV2.js';
import { logger } from '../utils/logger.js';
const V2 = ComponentsV2.IS_COMPONENTS_V2;
const EPH = MessageFlags.Ephemeral;
const activePolls = new Map();
function makeResultsProgressBar(votes, total, size = 10) {
    if (total === 0)
        return '░'.repeat(size);
...
function renderPollContainer(poll, ended = false) {
    ...
    if (!ended) {
        ...
        const endRow = new ActionRowBuilder().addComponents(new ButtonBuilder()
...
export const pollCommand = {
    ...
    async handleModal(interaction) {
        if (interaction.customId !== 'poll_wiz_modal')
            return;
    ...
    async handleButton(interaction) {
        if (!interaction.customId.startsWith('poll:end:'))
            return;
        const pollId = interaction.customId.split(':')[2];
        const poll = activePolls.get(pollId);
        if (!poll) {
