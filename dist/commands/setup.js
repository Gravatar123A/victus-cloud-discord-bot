import { SlashCommandBuilder, ChannelType, PermissionFlagsBits, AttachmentBuilder, MessageFlags, } from 'discord.js';
        ...
        try {
            switch (subcommand) {
                ...
                case 'resync-directory': {
                    const res = await forumDirectoryService.resyncGuild(interaction.guild.id);
                    const container = ComponentsV2.successContainer('Directory Resync Scheduled', `🔄 **Full directory synchronization initiated.**\n\n` +
                ...
                case 'status': {
                    ...
                    const container = ComponentsV2.baseContainer(ComponentsV2.Accents.info)
                        .addTextDisplayComponents(ComponentsV2.text(`# Discovery Forum Directory Status\n\n` +
                ...
                case 'dump-json': {
                    ...
                    const container = ComponentsV2.baseContainer(ComponentsV2.Accents.info)
                        .addTextDisplayComponents(ComponentsV2.text(`# Discovered Servers Export\n\n` +
                ...
                case 'hub-bridge': {
                    ...
                    if (channel.type !== ChannelType.GuildText && channel.type !== ChannelType.GuildAnnouncement) {
                        const err = ComponentsV2.errorContainer('Invalid Channel Type', `Channel <#${channel.id}> is not a text or announcement channel. Please select a standard text channel.`);
                        await interaction.editReply({ components: [err], flags: ComponentsV2.IS_COMPONENTS_V2 });
                        return;
                    }
                    const { webhookUrl, hubConnected } = await hubBridgeService.setupChannel(interaction.guild, channel);
                    const container = ComponentsV2.baseContainer(ComponentsV2.Accents.success)
                        .addTextDisplayComponents(ComponentsV2.text(`# 🌉 Main Hub Discord Bridge Connected\n\n` +
            ...
            const errContainer = ComponentsV2.errorContainer('Setup Operation Failed', `An error occurred: \`${err?.message || 'Unknown error'}\``);
