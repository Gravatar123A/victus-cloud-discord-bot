...
function renderWizPage(session) {
    const page = session.page;
    const c = ComponentsV2.baseContainer(ComponentsV2.Accents.primary);
    let title = `Custom Command Builder • Page ${page}/3`;
    let desc = '';
    const navRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`customcmd_wiz:prev:${page}`).setLabel('⬅️ Back').setStyle(ButtonStyle.Secondary).setDisabled(page === 1), new ButtonBuilder().setCustomId(`customcmd_wiz:next:${page}`).setLabel('Next ➡️').setStyle(ButtonStyle.Primary).setDisabled(page === 3), new ButtonBuilder().setCustomId(`customcmd_wiz:cancel`).setLabel('Cancel ❌').setStyle(ButtonStyle.Danger));
    const actionRows = [];
    switch (page) {
        case 1:
            ...
            actionRows.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('customcmd_wiz:modal:basic').setLabel('Edit Basic Info').setStyle(ButtonStyle.Secondary)));
            break;
        case 2:
            desc = `### Page 2: Permissions & Aliases\n` +
                `Bind roles permissions and alternate shortcut aliases.\n\n` +
                `› **Required Roles:** ${session.permissions.length > 0 ? session.permissions.map((r) => `<@&${r}>`).join(', ') : '*None (Open to everyone)*'}\n` +
                `› **Aliases:** ${session.aliases.length > 0 ? session.aliases.map((a) => `\`${a}\``).join(', ') : '*None*'}\n\n` +
                `Press the button below to configure filters.`;
            actionRows.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('customcmd_wiz:modal:perms').setLabel('Configure Filters').setStyle(ButtonStyle.Secondary)));
            break;
        case 3:
            ...
            actionRows.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('customcmd_wiz:modal:response').setLabel('Edit Response Content').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId('customcmd_wiz:save').setLabel('Save Command ✅').setStyle(ButtonStyle.Success).setDisabled(!session.name || !session.replyContent)));
            break;
    }
...
export const customcmdCommand = {
        ...
        if (sub === 'create') {
            ...
            list.forEach((c, i) => {
                text += `### \`#${i + 1}\` Command: **\`${c.name}\`**\n` +
                    ...
                    (c.aliases && c.aliases.length > 0 ? `› **Aliases:** ${c.aliases.map((a) => `\`${a}\``).join(', ')}\n` : '') +
    ...
    async handleButton(interaction) {
        if (!interaction.customId.startsWith('customcmd_wiz:'))
            return;
        const key = getSessionKey(interaction.user.id, interaction.guildId);
        ...
        if (action === 'prev') {
            ...
            if (target === 'basic') {
                ...
                modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('name').setLabel('Command Name').setPlaceholder('website').setValue(session.name).setStyle(TextInputStyle.Short).setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('cooldown').setLabel('Cooldown (Seconds)').setPlaceholder('5').setValue(String(session.cooldown)).setStyle(TextInputStyle.Short).setRequired(false)));
                ...
                modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('roles').setLabel('Required Roles IDs (CSV)').setPlaceholder('98273498237492,83274928374982').setValue(session.permissions.join(',')).setStyle(TextInputStyle.Paragraph).setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('aliases').setLabel('Aliases Shortcuts (CSV)').setPlaceholder('site,web').setValue(session.aliases.join(',')).setStyle(TextInputStyle.Short).setRequired(false)));
                await interaction.showModal(modal);
            }
    ...
    async handleSelectMenu(interaction) {
        if (interaction.customId !== 'customcmd_wiz:select:type')
            return;
        const key = getSessionKey(interaction.user.id, interaction.guildId);
        const session = wizardSessions.get(key);
        if (!session)
    ...
    async handleModal(interaction) {
        if (!interaction.customId.startsWith('customcmd_wiz_modal:'))
            return;
        const key = getSessionKey(interaction.user.id, interaction.guildId);
        const session = wizardSessions.get(key);
        if (!session) {
