...
import { BOT_BANNER_URL, createEmbed, successEmbed, errorEmbed, warningEmbed, } from '../../embeds/theme.js';
...
export const adminCommand = {
    data: new SlashCommandBuilder()
        ...
        .addChoices({ name: '📘 Info', value: 'info' }, { name: '✅ Success', value: 'success' }, { name: '⚠️ Warning', value: 'warning' }, { name: '❌ Error', value: 'error' }))
        ...
        .setRequired(true)
        .addChoices({ name: 'Set balance', value: 'set' }, { name: 'Add credits', value: 'add' }, { name: 'Remove credits', value: 'remove' }))
...
async function handleSearch(interaction) {
    ...
    const results = billingUsers.filter((u) => u.email?.toLowerCase().includes(query.toLowerCase()) ||
...
async function handleRoleSync(interaction) {
...
async function handleAnnounce(interaction) {
    ...
    if (channel) {
        try {
            ...
            await interaction.editReply({
                embeds: [
                    successEmbed('Announcement Posted', `Announcement has been posted to ${channel}.`),
                ],
            });
            // Log audit
            await supabase.logAudit(interaction.user.id, interaction.user.tag, 'announcement_created', 'announcement', title, { title, message, type, channel_id: channel.id });
        }
        catch (error) {
            await interaction.editReply({
...
async function handleForceLink(interaction) {
    ...
    await interaction.editReply({
        embeds: [
            successEmbed('Account Linked', `Linked <@${discordUser.id}> to Victus Cloud user \`${userId}\``),
        ],
    });
    await supabase.logAudit(interaction.user.id, interaction.user.tag, 'force_link', 'user', discordUser.id, { discord_id: discordUser.id, user_id: userId });
}
async function handleForceUnlink(interaction) {
    ...
    if (success) {
        const member = interaction.guild
            ...
            : null;
        if (member)
            await removeEntitlementRoles(member);
        await interaction.editReply({
            embeds: [
                successEmbed('Account Unlinked', `Unlinked <@${discordUser.id}> from Victus Cloud.`),
            ],
        });
        await supabase.logAudit(interaction.user.id, interaction.user.tag, 'force_unlink', 'user', discordUser.id, { discord_id: discordUser.id });
    }
    else {
        await interaction.editReply({
...
async function handleCredits(interaction) {
    ...
    const embed = successEmbed('Credits Updated', [
    ...
    await supabase.logAudit(interaction.user.id, interaction.user.tag, 'discord_admin_credits_adjusted', 'paymenter_user', String(result.user_id || resolvedTarget.user_id || resolvedTarget.email || target), {
        target,
    ...
    await createPagination(interaction, {
        ...
        embedBuilder: (items, page, totalPages) => {
            return new EmbedBuilder()
                ...
                ? items
                    .map((s) => `• **${s.name}** (\`${s.identifier}\`) - Node #${s.node}`)
                    .join('\n')
                : 'No servers found.')
                .setFooter({
...
async function handleStats(interaction) {
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: '🎮 Servers', value: `${servers.length}`, inline: true }, { name: '🖥️ Nodes', value: `${nodes.length}`, inline: true }, { name: '👥 Users', value: `${billingUsers.length}`, inline: true }, { name: '🧾 Invoices', value: `${invoices.length}`, inline: true }, { name: '💰 Revenue', value: `$${totalRevenue.toFixed(2)}`, inline: true }, { name: '✅ Paid', value: `${paidInvoices.length}`, inline: true })
        .setTimestamp();
    await interaction.editReply({ embeds: [embed] });
}
async function handleSetResourceForum(interaction) {
    ...
    await interaction.editReply({
        embeds: [
            successEmbed('Resource Forum Channel Configured', `Target forum channel set to ${channel}.\n\n` +
                `**Detected Forum Channel Tags:**\n${tagList}\n\n` +
                `Users can now run \`/share-resource\` to post resources directly into this forum channel.`),
        ],
    });
}
async function handleConfigTags(interaction) {
    ...
    await interaction.editReply({
        embeds: [
            successEmbed('Category Tag Mapping Updated', `Category **${category}** will now map to tags: ${tags.map((t) => `\`${t}\``).join(', ')}.`),
        ],
    });
}
