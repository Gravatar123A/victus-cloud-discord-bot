    ...
    async execute(interaction) {
        ...
        if (existingLink) {
            await interaction.editReply({
                components: [
                    ComponentsV2.infoContainer('Account Already Connected', 'Your Discord account is already linked to Victus Cloud.\n\nUse `/account` to view your linked profile.'),
                ],
        ...
        if (!linkToken) {
            await interaction.editReply({
                components: [
                    ComponentsV2.errorContainer('Signup Link Failed', 'Could not create your secure signup link. Please try again in a moment.'),
                ],
                flags: ComponentsV2.IS_COMPONENTS_V2,
            });
            return;
        ...
        const container = ComponentsV2.linkAccountContainer(interaction.user.tag, interaction.user.displayAvatarURL({ size: 128 }), expiryTimestamp, signupUrl);
