...
export const manualDcLinkCommand = {
    data: new SlashCommandBuilder()
        .setName('manualdclink')
        .setDescription('Manually link a Discord user to a Victus Cloud account (admin only)')
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
        .addUserOption((opt) => opt.setName('user').setDescription('Discord user to link').setRequired(true))
        .addStringOption((opt) => opt.setName('email').setDescription('Victus Cloud email (exact, case-insensitive)').setRequired(true)),
    adminOnly: true,
    cooldown: 5,
    async execute(interaction) {
        ...
        if (existingVictus) {
            await interaction.editReply({
                components: [ComponentsV2.warningContainer('Victus Account Already Linked', `Victus account \`${targetEmail}\` is already linked to <@${existingVictus.discord_id}> (\`${existingVictus.discord_id}\`). Unlink it first with \`/admin unlink\`.`)],
                flags: ComponentsV2.IS_COMPONENTS_V2,
            });
            return;
        }
        ...
        try {
            await assignLinkedRole(interaction.client, discordUser.id);
        }
        catch (e) {
            logger.warn(`manualdclink assignLinkedRole failed for ${discordUser.id}:`, e);
        }
        try {
            await syncEntitlementRoles(interaction.client, discordUser.id);
        }
        catch (e) {
            logger.warn(`manualdclink syncEntitlementRoles failed for ${discordUser.id}:`, e);
        }
        try {
            await supabase.grantDiscordLinkCoins(linked);
        }
        catch (e) {
            logger.warn(`manualdclink grantDiscordLinkCoins failed for ${discordUser.id}:`, e);
        }
        const success = ComponentsV2.successContainer('Manually Linked', `Linked <@${discordUser.id}> (\`${discordUser.tag ?? discordUser.username}\`) → Victus \`${targetEmail}\` (\`${targetUserId}\`)\n\n` +
        ...
        try {
            await sendNotificationDM(interaction.client, discordUser.id, ComponentsV2.successContainer('Account Linked by Admin', `Your Discord account was manually linked to Victus Cloud \`${targetEmail}\` by an admin. You now have access to linked commands!`), 'security');
        }
        catch { }
        if (interaction.guildId) {
            await sendAuditLog(interaction.client, interaction.guildId, 'Account Manually Linked', `**Admin:** <@${interaction.user.id}> (\`${interaction.user.tag}\`)\n**Discord:** <@${discordUser.id}> (\`${discordUser.id}\`)\n**Victus:** \`${targetEmail}\` (\`${targetUserId}\`)`, ComponentsV2.Accents.success).catch(() => { });
        }
