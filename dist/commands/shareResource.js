import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelType, EmbedBuilder, MessageFlags, ModalBuilder, SlashCommandBuilder, StringSelectMenuBuilder, TextInputBuilder, TextInputStyle, } from 'discord.js';
import { ComponentsV2 } from '../embeds/componentsV2.js';
import { scrapeResourceUrl } from '../services/resourceScraper.js';
import { resourceSessionStore } from '../services/resourceSessionStore.js';
 ...
 */
function buildResourcePreviewComponents(session) {
...
export const shareResourceCommand = {
    ...
    async execute(interaction) {
        if (!interaction.guildId)
            return;
        const container = ComponentsV2.infoContainer('Share a Resource', 'Welcome to the Victus Cloud Resource Hub! Choose how you would like to share your resource:\n\n' +
    ...
    async handleButton(interaction) {
        ...
        if (!customId.startsWith('victus_res_btn_'))
            return;
        if (!interaction.guildId)
            return;
        ...
        if (customId === 'victus_res_btn_manual') {
            ...
            const tagsInput = new TextInputBuilder()
                ...
                .setStyle(TextInputStyle.Short)
                .setRequired(false);
            modal.addComponents(new ActionRowBuilder().addComponents(titleInput), new ActionRowBuilder().addComponents(descInput), new ActionRowBuilder().addComponents(linkInput), new ActionRowBuilder().addComponents(authorInput), new ActionRowBuilder().addComponents(tagsInput));
            await interaction.showModal(modal);
            return;
        }
        ...
        if (customId.startsWith('victus_res_btn_like:')) {
            ...
            const updatedLikeButton = new ButtonBuilder()
                ...
                .setEmoji('❤️')
                .setStyle(ButtonStyle.Secondary);
            const updatedRow = new ActionRowBuilder().addComponents(updatedLikeButton);
            if (listing.sourceUrl) {
                updatedRow.addComponents(new ButtonBuilder()
            ...
            const rewardResult = await supabase.grantResourceLikeCoins(listing.userId, LIKE_REWARD_COINS, `resource_like:${listing.id}:${interaction.user.id}`);
                ...
                if (ownerUser) {
                    if (rewardResult.success) {
                        await ownerUser.send({
                            components: [
                                ComponentsV2.cleanContainer(ComponentsV2.Accents.success, 'Resource Liked! +20 Coins', `🎉 <@${interaction.user.id}> liked your resource **${listing.title}**!\n\n` +
                                    `**+${LIKE_REWARD_COINS} Victus Coins** have been credited directly to your Victus Cloud account balance! 💰\n\n` +
                                    `👉 [View Your Resource Forum Post](${listing.threadUrl})`, 'COIN REWARD'),
                            ],
                        ...
                        await ownerUser.send({
                            components: [
                                ComponentsV2.cleanContainer(ComponentsV2.Accents.warning, 'Resource Liked! Link Account to Claim Coins', `🎉 <@${interaction.user.id}> liked your resource **${listing.title}**!\n\n` +
                                    `You would have earned **+${LIKE_REWARD_COINS} Victus Coins**, but your Discord account is not linked to Victus Cloud.\n\n` +
                                    `Run \`/link\` to connect your account and start earning rewards!`, 'ACTION REQUIRED'),
                            ],
                            flags: ComponentsV2.IS_COMPONENTS_V2,
                        }).catch(() => { });
                    }
        ...
        if (!session) {
            await interaction.reply({
                components: [
                    ComponentsV2.warningContainer('Session Expired', 'Your resource sharing session has expired. Please run `/share-resource` again.'),
                ],
                flags: MessageFlags.Ephemeral | ComponentsV2.IS_COMPONENTS_V2,
            });
            return;
        ...
        if (customId === 'victus_res_btn_edit') {
            ...
            const authorInput = new TextInputBuilder()
                ...
                .setStyle(TextInputStyle.Short)
                .setRequired(false);
            modal.addComponents(new ActionRowBuilder().addComponents(titleInput), new ActionRowBuilder().addComponents(descInput), new ActionRowBuilder().addComponents(authorInput));
            await interaction.showModal(modal);
            return;
        }
        ...
        if (customId === 'victus_res_btn_tags_links') {
            ...
            const imageInput = new TextInputBuilder()
                ...
                .setStyle(TextInputStyle.Short)
                .setRequired(false);
            modal.addComponents(new ActionRowBuilder().addComponents(sourceUrlInput), new ActionRowBuilder().addComponents(tagsInput), new ActionRowBuilder().addComponents(imageInput));
            await interaction.showModal(modal);
            return;
        }
        // 6. Cancel Button
        if (customId === 'victus_res_btn_cancel') {
            ...
            await interaction.update({
                components: [
                    ComponentsV2.infoContainer('Resource Sharing Cancelled', 'The draft resource sharing session has been cancelled.'),
                ],
            });
            return;
        }
        ...
        if (customId === 'victus_res_btn_submit') {
            ...
            const postEmbed = new EmbedBuilder()
                ...
                .addFields({ name: '📁 Category', value: session.category, inline: true }, { name: '👤 Creator', value: session.author || 'Community', inline: true }, { name: '🔗 Source Page', value: session.sourceUrl ? `[View Original Link](${session.sourceUrl})` : 'N/A', inline: false });
            ...
            try {
                ...
                try {
                    await interaction.user.send(`🎉 **Your resource listing "${session.title}" has been published!**\n\n` +
                ...
                await interaction.editReply({
                    components: [
                        ComponentsV2.successContainer('Resource Published!', `Your resource **${session.title}** has been successfully published to the forum!\n\n` +
                ...
                await interaction.editReply({
                    components: [
                        ComponentsV2.errorContainer('Posting Failed', `Failed to create forum post: ${error?.message || 'Check bot permissions for the forum channel.'}`),
                    ],
                });
            }
        }
    ...
    async handleSelectMenu(interaction) {
        if (interaction.customId !== 'victus_res_select_category')
            return;
        if (!interaction.guildId)
            return;
        const userId = interaction.user.id;
    ...
    async handleModal(interaction) {
        const customId = interaction.customId;
        if (!customId.startsWith('victus_res_modal_'))
            return;
        if (!interaction.guildId)
            return;
        const userId = interaction.user.id;
        ...
        if (customId === 'victus_res_modal_url') {
            ...
            const renderLoading = async () => {
                if (isComplete)
                    return;
                try {
                    const container = ComponentsV2.resourceLoadingContainer(rawUrl, currentPercent, currentStage, currentDetail);
            ...
            const tickerInterval = setInterval(() => {
                if (isComplete || tickerIndex >= TICKER_STEPS.length)
                    return;
