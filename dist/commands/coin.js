import { ActionRowBuilder, ButtonBuilder, ButtonStyle, SlashCommandBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, MessageFlags, ContainerBuilder, } from 'discord.js';
...
function sourceMeta(source) {
    ...
    if (exact)
        return exact;
    if (source.endsWith('_wager'))
        return { emoji: '🎲', label: 'Game Wager', color: '#ef4444' };
    if (source.endsWith('_win'))
        return { emoji: '🏆', label: 'Game Win', color: '#22c55e' };
    if (source.endsWith('_refund'))
        return { emoji: '↩️', label: 'Game Refund', color: '#3b82f6' };
    if (source.endsWith('_target_loss'))
        return { emoji: '🥷', label: 'Heist Loss', color: '#ef4444' };
    if (source.endsWith('_team_payout'))
        return { emoji: '🥷', label: 'Heist Share', color: '#22c55e' };
    if (source.startsWith('gambling'))
        return SOURCE_META.gambling;
...
async function fetchAllCoinHistory(userId, email, periodDays) {
    ...
    const currentBalance = live?.found ? Number(live.coins) : Number(profile?.total_cp ?? 0);
    // Fetch from multiple sources in parallel
    const [economyLedger, cpTxs, panelTxs] = await Promise.all([
        supabase.getEconomyLedger(userId, 100, 0).catch(() => []),
        supabase.getCpTransactions(userId, 100, 0).catch(() => []),
        fetchPanelCreditTransactions(userId, email, since),
    ]);
    const unified = [];
    // Economy ledger (Coins transfers, bank, admin adjusts, etc.)
    for (const r of economyLedger) {
...
async function fetchPanelCreditTransactions(userId, email, since) {
...
function buildHistoryContainer(discordId, profile, period, txs, currentBalance, page, hasNext, hasPrev) {
    ...
    header.addTextDisplayComponents({
        type: 10,
        content: `# 🧾 Coin History — ${periodLabel}\n` +
    ...
    // Period selector
    const periodRow = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
    ...
    header.addActionRowComponents(periodRow);
    // Pagination
    const btnRow = new ActionRowBuilder().addComponents(new ButtonBuilder()
        ...
        .setEmoji('◀️')
        .setDisabled(!hasPrev), new ButtonBuilder().setCustomId(`coin:dash:${discordId}`).setLabel('Dashboard').setStyle(ButtonStyle.Secondary).setEmoji('🏠'), new ButtonBuilder()
        .setCustomId(`coin:hist:${discordId}:${period}:${page + 1}`)
        .setLabel('Next')
        .setStyle(ButtonStyle.Secondary)
        .setEmoji('▶️')
        .setDisabled(!hasNext), new ButtonBuilder().setCustomId(`coin:refresh:${discordId}:${period}:${page}`).setLabel('Refresh').setStyle(ButtonStyle.Primary).setEmoji('🔄'));
    header.addActionRowComponents(btnRow);
    // Quick filter buttons
    const filterRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`coin:filter:${discordId}:${period}:${page}:all`).setLabel('All').setStyle(ButtonStyle.Secondary).setEmoji('📋'), new ButtonBuilder().setCustomId(`coin:filter:${discordId}:${period}:${page}:gain`).setLabel('Gains').setStyle(ButtonStyle.Success).setEmoji('📈'), new ButtonBuilder().setCustomId(`coin:filter:${discordId}:${period}:${page}:loss`).setLabel('Losses').setStyle(ButtonStyle.Danger).setEmoji('📉'), new ButtonBuilder().setCustomId(`coin:export:${discordId}:${period}`).setLabel('Export CSV').setStyle(ButtonStyle.Secondary).setEmoji('📄'));
    header.addActionRowComponents(filterRow);
    return header;
}
export const coinCommand = {
    data: new SlashCommandBuilder()
        ...
        .addChoices({ name: 'Last 7 days', value: '7d' }, { name: 'Last 30 days', value: '30d' }, { name: 'Last 90 days', value: '90d' }, { name: 'Last 180 days', value: '180d' }, { name: 'Last year', value: '1y' }, { name: 'All time', value: 'all' })
        .setRequired(false))
        .addStringOption((opt) => opt.setName('type').setDescription('Filter by type').addChoices({ name: 'All', value: 'all' }, { name: 'Gains only', value: 'gain' }, { name: 'Losses only', value: 'loss' }).setRequired(false))
    ...
    async execute(interaction) {
        ...
        if (targetUser) {
            ...
            const isAdmin = Boolean(requesterProfile?.is_admin);
            if (!isAdmin) {
                await interaction.reply({
                    components: [new ContainerBuilder().setAccentColor(VICTUS_COLORS.error).addTextDisplayComponents({ type: 10, content: `# ⛔ Admin only\nYou can only view your own coin history. Admins can view others.` })],
                    flags: V2,
                    ephemeral: true,
                });
                return;
        ...
        await interaction.deferReply({ flags: V2, ephemeral: true });
        try {
            ...
            if (!profile) {
                await interaction.editReply({
                    components: [
                        new ContainerBuilder().setAccentColor(VICTUS_COLORS.error).addTextDisplayComponents({ type: 10, content: `# ❌ Profile not found\nCould not load profile for <@${discordId}>.` }),
                    ],
                    flags: V2,
                });
                return;
            ...
            await interaction.editReply({
                components: [new ContainerBuilder().setAccentColor(VICTUS_COLORS.error).addTextDisplayComponents({ type: 10, content: `# ❌ Error\n${msg.slice(0, 2000)}` })],
                flags: V2,
            });
        }
    },
    async handleButton(interaction) {
        ...
        if (owner && interaction.user.id !== owner) {
            await interaction.reply({ content: 'That panel belongs to someone else — run `/coin history` to open your own.', flags: MessageFlags.Ephemeral });
            return;
        }
        ...
        if (action === 'dash') {
            ...
            await interaction.update({ components: [new ContainerBuilder().setAccentColor(VICTUS_COLORS.primary).addTextDisplayComponents({ type: 10, content: `# 🏠 Back to Economy\nRun \`/economy\` for your full dashboard.` })], flags: V2 });
            return;
        }
        if (action === 'hist') {
            ...
            try {
                const linked = await supabase.getLinkedAccount(discordId).catch(() => null);
                if (!linked?.user_id) {
                    await interaction.editReply({ components: [new ContainerBuilder().setAccentColor(VICTUS_COLORS.warning).addTextDisplayComponents({ type: 10, content: `# 🔗 Link Required\nRun \`/link\` to view history.` })], flags: V2 });
                    return;
                }
                const profile = await supabase.getUserProfile(linked.user_id).catch(() => null);
                const periodDays = PERIODS.find((p) => p.value === period)?.days ?? 30;
                const { txs, currentBalance } = await fetchAllCoinHistory(linked.user_id, profile?.email || '', periodDays);
                const start = page * PAGE_SIZE;
                ...
                await interaction.editReply({ components: [new ContainerBuilder().setAccentColor(VICTUS_COLORS.error).addTextDisplayComponents({ type: 10, content: `# ❌ Error\n${e.message}` })], flags: V2 });
            }
            return;
        }
        if (action === 'refresh') {
        ...
        if (action === 'export') {
            ...
            try {
                const linked = await supabase.getLinkedAccount(discordId).catch(() => null);
                if (!linked?.user_id) {
                    await interaction.editReply({ components: [new ContainerBuilder().setAccentColor(VICTUS_COLORS.warning).addTextDisplayComponents({ type: 10, content: `# 🔗 Link Required\nRun \`/link\` to export history.` })], flags: V2 });
                    return;
                }
                const profile = await supabase.getUserProfile(linked.user_id).catch(() => null);
                const periodDays = PERIODS.find((p) => p.value === period)?.days ?? 30;
                const { txs } = await fetchAllCoinHistory(linked.user_id, profile?.email || '', periodDays);
                const csv = [
                ...
                await interaction.editReply({
                    components: [new ContainerBuilder().setAccentColor(VICTUS_COLORS.success).addTextDisplayComponents({ type: 10, content: `# 📄 Export Ready\nExported **${txs.length}** transactions for **${period}** as CSV.` })],
                    files: [{ attachment: buffer, name: `coin-history-${period}-${Date.now()}.csv` }],
                    flags: V2,
                });
            }
            catch (e) {
                await interaction.editReply({ components: [new ContainerBuilder().setAccentColor(VICTUS_COLORS.error).addTextDisplayComponents({ type: 10, content: `# ❌ Export failed\n${e.message}` })], flags: V2 });
            }
            return;
        }
    },
