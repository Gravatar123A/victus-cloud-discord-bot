...
function renderJ2CDashboard(config) {
    ...
    const hubSelect = new ActionRowBuilder().addComponents(new ChannelSelectMenuBuilder()
        .setCustomId('j2c_wiz:hub_channel')
        .setPlaceholder('Select hub voice channel (trigger)...')
        .addChannelTypes(ChannelType.GuildVoice));
    // Row 2: Select Parent Category
    const categorySelect = new ActionRowBuilder().addComponents(new ChannelSelectMenuBuilder()
...
export function buildVoiceControlPanel(ownerId) {
...
export const j2cCommand = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub => sub.setName('setup').setDescription('Open the Join to Create voice system setup wizard')),
    ...
    async handleModal(interaction) {
        ...
        if (interaction.customId.startsWith('j2c_panel_modal:')) {
            ...
            if (action === 'rename') {
                ...
                const targetMember = voiceChannel.members.find((m) => m.id === input || m.user.username.toLowerCase() === input || m.user.tag.toLowerCase() === input);
                if (!targetMember) {
                ...
                const input = interaction.fields.getTextInputValue('userId').trim().toLowerCase();
                const targetMember = interaction.guild?.members.cache.find((m) => m.id === input || m.user.username.toLowerCase() === input || m.user.tag.toLowerCase() === input);
                if (!targetMember) {
                ...
                await interaction.message.edit({ components: [updatedPanel] });
