import { SlashCommandBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder, ContainerBuilder, MessageFlags, } from 'discord.js';
...
export const serversCommand = {
    ...
    async autocomplete(interaction) {
        ...
        try {
            ...
            const filtered = servers
                .filter((server) => decodeDisplayText(server.name, '').toLowerCase().includes(focused) ||
                String(server.identifier || '').toLowerCase().includes(focused))
                .slice(0, 25);
            await interaction.respond(filtered.map((server) => ({
    ...
    async handleButton(interaction) {
        ...
        const server = servers.find((candidate) => candidate.identifier === serverId);
        if (!server) {
            await interaction.editReply({
        ...
        try {
            ...
            await interaction.editReply({
                components: [
                    ComponentsV2.successContainer('Power Signal Sent', `Sent **${action}** to **${decodeDisplayText(server.name)}**.\n\nServer state should update shortly.`),
                ],
                flags: ComponentsV2.IS_COMPONENTS_V2,
            });
        }
...
async function handleServerList(interaction, userId) {
    const servers = await getServersForUser(userId);
    if (servers.length === 0) {
        const container = new ContainerBuilder()
            .setAccentColor(ComponentsV2.Accents.info)
            .addTextDisplayComponents(ComponentsV2.text(`# ${Icons.server} Your Servers\n\n` +
    ...
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(ComponentsV2.text(`# ${Icons.crown} Your Server Fleet\n-# ${servers.length} server${servers.length !== 1 ? 's' : ''} connected to your Victus account`))
        .addSeparatorComponents(ComponentsV2.separator());
    servers.slice(0, 10).forEach((server, index) => {
        const status = getServerStatus(server);
        container.addTextDisplayComponents(ComponentsV2.text(`### ${statusIcon(status)} ${decodeDisplayText(server.name)}\n` +
    ...
    const menu = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
        ...
        .addOptions(servers.slice(0, 25).map((server) => ({
...
async function handleServerInfo(interaction, userId) {
    ...
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(ComponentsV2.text(`# ${icon} ${decodeDisplayText(server.name)}\n` +
        `\`${server.identifier}\`\n\n` +
        `${decodeDisplayText(server.description, '_No description_')}`))
        .addSeparatorComponents(ComponentsV2.separator())
        .addTextDisplayComponents(ComponentsV2.text(`### ${Icons.activity} Live State\n` +
        `${icon} **Status:** ${statusLabel(status)}\n` +
        `${Icons.node} **Node:** #${server.node}\n` +
        `${Icons.network} **Allocation:** #${server.allocation || 'Unknown'}`))
        .addSeparatorComponents(ComponentsV2.separator())
        .addTextDisplayComponents(ComponentsV2.text(`### ${Icons.spark} Resource Envelope\n` +
        ...
        .addSeparatorComponents(ComponentsV2.separator())
        .addTextDisplayComponents(ComponentsV2.text(`### ${Icons.service} Feature Limits\n` +
    ...
    const buttons = new ActionRowBuilder().addComponents(new ButtonBuilder()
        ...
        .setDisabled(status === 'running'), new ButtonBuilder()
        .setCustomId(`server_stop_${serverId}`)
        .setLabel('Stop')
        .setStyle(ButtonStyle.Danger)
        .setDisabled(status === 'offline' || status === 'stopped'), new ButtonBuilder()
...
async function handleServerPower(interaction, userId) {
    const serverId = interaction.options.getString('server', true);
    ...
    try {
        ...
        await interaction.editReply({
            components: [
                ComponentsV2.errorContainer('Power Action Failed', `I found **${decodeDisplayText(server.name)}**, but could not send **${action}**.\n\n${message}`),
            ],
            flags: ComponentsV2.IS_COMPONENTS_V2,
        });
        return;
    ...
    await interaction.editReply({
        components: [
            ComponentsV2.successContainer(`${actionIcon[action]} Power Signal Sent`, `Sent **${action}** to **${decodeDisplayText(server.name)}**.\n\nServer state should update shortly.`),
        ],
        flags: ComponentsV2.IS_COMPONENTS_V2,
    });
}
