import { ActionRowBuilder, ButtonBuilder, ButtonStyle, MessageFlags, PermissionFlagsBits, SlashCommandBuilder, StringSelectMenuBuilder, EmbedBuilder } from 'discord.js';
...
function renderWhitelistEditor(record) {
...
export const whitelistCommand = {
    ...
    async handleSelectMenu(interaction) {
        if (!interaction.customId.startsWith('whitelist:select:'))
            return;
        const targetId = interaction.customId.split(':')[2];
        const guildId = interaction.guildId;
        const config = await whitelistSettings.get(guildId);
