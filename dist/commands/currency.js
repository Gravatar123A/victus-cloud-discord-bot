import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ModalBuilder, MessageFlags, SlashCommandBuilder, TextInputBuilder, TextInputStyle, } from 'discord.js';
...
async function fetchConversion(amount, from, to) {
    ...
    try {
        const response = await fetch(`https://api.frankfurter.app/latest?amount=${encodeURIComponent(amount)}&from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`, { signal: controller.signal });
...
function converterPanel() {
    const container = ComponentsV2.cleanContainer(ComponentsV2.Accents.info, 'Currency converter', '**Convert between supported currencies with current reference rates.**\n\n' +
        'Enter any ISO 4217 currency codes such as `USD`, `EUR`, `GBP`, `AED`, `PKR`, or `JPY`.\n\n' +
        '-# Rates are provided by Frankfurter and are for estimates and discussion only.', 'FINANCE UTILITY');
...
export const currencyCommand = {
    ...
    async handleButton(interaction) {
        if (interaction.customId !== CURRENCY_BUTTON)
            return;
        await interaction.showModal(currencyModal());
    },
    async handleModal(interaction) {
        if (interaction.customId !== CURRENCY_MODAL)
            return;
        ...
        try {
            ...
            await interaction.reply({
                components: [ComponentsV2.successContainer('Conversion complete', `**${amount.toLocaleString(undefined, { maximumFractionDigits: 8 })} ${from}** = **${converted.toLocaleString(undefined, { maximumFractionDigits: 8 })} ${to}**\n\n` +
            ...
            await interaction.reply({
                content: `Unable to convert those currencies: ${error.message}`,
