 ...
 */
import { ChannelType, MessageFlags, PermissionFlagsBits, SlashCommandBuilder, EmbedBuilder, ActionRowBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, StringSelectMenuBuilder, ContainerBuilder, TextDisplayBuilder, SeparatorBuilder, } from 'discord.js';
import { config } from '../../config.js';
import { logger } from '../../utils/logger.js';
import { ComponentsV2 } from '../../embeds/componentsV2.js';
import { addedContainer, musicIdleContainer, nowPlayingContainer, queueContainer, musicControlsContainer, escapeMd, } from '../../embeds/music.js';
...
async function requireVoice(interaction, deferred = true) {
    const member = interaction.member;
    const voice = member?.voice?.channel ?? null;
    const fail = async (title, body) => {
...
async function requirePlayer(interaction, deferred = true) {
...
export const playCommand = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((o) => o.setName('query').setDescription('Song name, Spotify/YouTube/SoundCloud URL, or playlist link').setRequired(true).setMaxLength(500)),
    async execute(interaction) {
        await interaction.deferReply({ flags: V2 });
        const ctx = await requireVoice(interaction);
        if (!ctx)
            return;
        if (!(await requireMusicServer(interaction)))
            return;
        ...
        try {
            ...
            const searchQuery = isUrl
                ? { query }
                : { query, source: config.lavalink.defaultSource };
            ...
            if (isSpotify && (!res || !res.tracks?.length || res.loadType === 'empty' || res.loadType === 'error')) {
                ...
                res = await player.search({ query, source: 'spsearch' }, interaction.user);
            }
        }
        catch (error) {
    ...
    async handleSelectMenu(interaction) {
        if (interaction.customId !== 'music:controls')
            return;
        const player = interaction.client.lavalink.getPlayer(interaction.guildId);
        if (!player) {
    ...
    async handleModal(interaction) {
        if (interaction.customId !== 'music:volume_modal')
            return;
        const player = interaction.client.lavalink.getPlayer(interaction.guildId);
        if (!player) {
            await interaction.reply({ content: '❌ Nothing is playing right now.', ephemeral: true });
...
export const queueCommand = {
    ...
    async execute(interaction) {
        ...
        if (sub === 'edit') {
            ...
            c.addTextDisplayComponents(ComponentsV2.text(`# 📝 Edit Playlist Queue\n` +
                `Select a track from the dropdown select menu below to remove it from the queue.` +
                (tracks.length > 25 ? `\n\n_-# Showing first 25 tracks of ${tracks.length}_` : ''))).addSeparatorComponents(ComponentsV2.separator());
    ...
    async handleSelectMenu(interaction) {
        if (interaction.customId !== 'queue_edit:remove')
            return;
        const player = interaction.client.lavalink.getPlayer(interaction.guildId);
        if (!player) {
...
export const loopCommand = {
    data: new SlashCommandBuilder()
        ...
        .setDMPermission(false)
        .addStringOption((o) => o.setName('mode').setDescription('Loop mode').setRequired(true).addChoices({ name: 'Off', value: 'off' }, { name: 'Current track', value: 'track' }, { name: 'Whole queue', value: 'queue' })),
    async execute(interaction) {
        await interaction.deferReply({ flags: V2 });
        const player = await requirePlayer(interaction);
        if (!player)
...
const playrandomSessions = new Map();
function renderPlayrandomDashboard(userId, lang) {
    ...
    container.addActionRowComponents(new ActionRowBuilder().addComponents(langSelect));
    container.addActionRowComponents(new ActionRowBuilder().addComponents(catSelect));
    return container;
}
export const playrandomCommand = {
    data: new SlashCommandBuilder()
        .setName('playrandom')
