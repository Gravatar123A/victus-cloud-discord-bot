 ...
 */
import { SlashCommandBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, ContainerBuilder, MessageFlags, } from 'discord.js';
...
export const preferencesCommand = {
    ...
    async handleButton(interaction) {
        ...
        if (!customId.startsWith('pref_toggle_'))
            return;
        const linked = await requireLinkedAccount(interaction);
        if (!linked)
            return;
...
async function showNotificationPreferences(interaction, linked, isUpdate = false) {
    ...
    const getButtonStyle = (enabled) => enabled ? ButtonStyle.Success : ButtonStyle.Secondary;
    const container = new ContainerBuilder()
        .setAccentColor(ComponentsV2.Accents.info)
        .addTextDisplayComponents(ComponentsV2.text(`# Notification Preferences\n\n` +
    ...
    const buttons1 = new ActionRowBuilder().addComponents(new ButtonBuilder()
        .setCustomId(CUSTOM_IDS.TOGGLE_MAINTENANCE)
        .setLabel('Maintenance')
        .setStyle(getButtonStyle(resolvedPrefs.dm_maintenance)), new ButtonBuilder()
    ...
    const buttons2 = new ActionRowBuilder().addComponents(new ButtonBuilder()
        ...
        .setLabel('Security')
        .setStyle(getButtonStyle(resolvedPrefs.dm_security)), new ButtonBuilder()
