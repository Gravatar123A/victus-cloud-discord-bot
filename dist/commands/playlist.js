import { ActionRowBuilder, EmbedBuilder, SlashCommandBuilder, StringSelectMenuBuilder, ModalBuilder, TextInputBuilder, TextInputStyle } from 'discord.js';
...
export const playlistCommand = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub => sub.setName('rename')
    ...
    async execute(interaction) {
        ...
        try {
            if (sub === 'create') {
                ...
                let res;
                try {
                    const searchQuery = isUrl ? { query } : { query, source: config.lavalink.defaultSource };
                    ...
                    if (isSpotify && (!res || !res.tracks?.length || res.loadType === 'empty' || res.loadType === 'error')) {
                        res = await node.search({ query, source: 'spsearch' }, interaction.user);
                    }
                }
                catch (err) {
                ...
                for (const t of playlist.tracks) {
                    try {
                        ...
                        if ((!searchRes || !searchRes.tracks?.length || searchRes.loadType === 'empty' || searchRes.loadType === 'error') && t.source === 'spotify') {
                            searchRes = await player.search({ query: t.uri, source: 'spsearch' }, interaction.user);
                        }
                        ...
                        if (!searchRes || !searchRes.tracks?.length || searchRes.loadType === 'empty' || searchRes.loadType === 'error') {
                            ...
                            searchRes = await player.search({ query: fallbackQuery, source: config.lavalink.defaultSource }, interaction.user);
                        }
    ...
    async handleSelectMenu(interaction) {
        ...
        if (interaction.customId === 'playlist_edit:select_playlist') {
            ...
            const actionMenu = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
            ...
            if (action === 'delete') {
                ...
                const trackSelectMenu = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
