...
function renderAuditLogDashboard(config) {
    ...
    const eventsSelect = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
...
export const auditLogCommand = {
    ...
    async handleButton(interaction) {
        if (interaction.customId !== 'audit_log:toggle')
            return;
        const config = await auditLogSettings.get(interaction.guildId);
    ...
    async handleSelectMenu(interaction) {
        if (!interaction.customId.startsWith('audit_log:'))
            return;
