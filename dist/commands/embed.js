import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelType, MessageFlags, ModalBuilder, PermissionFlagsBits, SlashCommandBuilder, StringSelectMenuBuilder, TextInputBuilder, TextInputStyle } from 'discord.js';
...
function renderEditorDashboard(session) {
    ...
    const buttonsRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('embed_edit:save').setLabel('Save & Publish').setStyle(ButtonStyle.Success).setEmoji('💾'), new ButtonBuilder().setCustomId('embed_edit:cancel').setLabel('Cancel').setStyle(ButtonStyle.Danger).setEmoji('❌'));
    container.addActionRowComponents(new ActionRowBuilder().addComponents(selectMenu));
    container.addActionRowComponents(buttonsRow);
    return container;
}
function renderEditorButtonsPage(session) {
    const accent = session.color ? (PRESET_COLORS[session.color.toLowerCase()] || parseInt(session.color.replace('#', ''), 16) || ComponentsV2.Accents.primary) : ComponentsV2.Accents.primary;
    const container = ComponentsV2.baseContainer(accent);
    const body = `# 🖱️ Edit Custom Buttons: **\`${session.originalName}\`**\n` +
        ...
        (session.buttons && session.buttons.length > 0 ? session.buttons.map((b, i) => `  \`${i + 1}.\` **${b.label}** (${b.style === 5 ? `Link: ${b.url}` : `Linked Embed/Action: ${b.url || 'None'}`})`).join('\n') : '  _No buttons defined._') +
        `\n\nChoose a style below to add a button, or clear all buttons.`;
    container.addTextDisplayComponents(ComponentsV2.text(body));
    const buttonSelect = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
    ...
    const btnRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('embed_edit:clear_buttons').setLabel('Clear Buttons').setStyle(ButtonStyle.Danger).setDisabled(!session.buttons || session.buttons.length === 0), new ButtonBuilder().setCustomId('embed_edit:back_to_dashboard').setLabel('⬅️ Back to Editor').setStyle(ButtonStyle.Primary));
    container.addActionRowComponents(buttonSelect).addActionRowComponents(btnRow);
    return container;
}
function renderEditorSelectPage(session) {
    const accent = session.color ? (PRESET_COLORS[session.color.toLowerCase()] || parseInt(session.color.replace('#', ''), 16) || ComponentsV2.Accents.primary) : ComponentsV2.Accents.primary;
    const container = ComponentsV2.baseContainer(accent);
    const body = `# 📊 Edit Dropdown Select Menu: **\`${session.originalName}\`**\n` +
        `Configure a dropdown select menu below your embed.\n\n` +
        `› **Dropdown Menu:** ${session.selectMenu ? `**${session.selectMenu.options.length} options**` : '_None_'}\n` +
        (session.selectMenu ? `  Placeholder: "${session.selectMenu.placeholder}"\n` + session.selectMenu.options.map((o, i) => `  \`${i + 1}.\` **${o.label}** (${o.value})`).join('\n') : '') +
        `\n\nClick the buttons below to set or clear the dropdown.`;
    container.addTextDisplayComponents(ComponentsV2.text(body));
    const btnRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('embed_edit:modal_select_trigger').setLabel('Set Select Menu').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId('embed_edit:clear_select').setLabel('Clear Menu').setStyle(ButtonStyle.Danger).setDisabled(!session.selectMenu), new ButtonBuilder().setCustomId('embed_edit:back_to_dashboard').setLabel('⬅️ Back to Editor').setStyle(ButtonStyle.Primary));
    container.addActionRowComponents(btnRow);
    return container;
}
function renderEditorColorPage(session) {
    ...
    container.addTextDisplayComponents(ComponentsV2.text(body));
    const colorSelect = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
    ...
    const btnRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('embed_edit:modal_hex_trigger').setLabel('Enter Custom HEX').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId('embed_edit:back_to_dashboard').setLabel('⬅️ Back to Editor').setStyle(ButtonStyle.Primary));
    container.addActionRowComponents(colorSelect).addActionRowComponents(btnRow);
    return container;
}
function renderWizardPage(session) {
    const page = session.page;
    const accent = session.color ? (PRESET_COLORS[session.color.toLowerCase()] || parseInt(session.color.replace('#', ''), 16) || ComponentsV2.Accents.primary) : ComponentsV2.Accents.primary;
    const container = ComponentsV2.baseContainer(accent);
    let title = `Embed Builder • Page ${page}/7`;
    let desc = '';
    const navRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`embed_wiz:prev:${page}`).setLabel('⬅️ Back').setStyle(ButtonStyle.Secondary).setDisabled(page === 1), new ButtonBuilder().setCustomId(`embed_wiz:next:${page}`).setLabel('Next ➡️').setStyle(ButtonStyle.Primary).setDisabled(page === 7), new ButtonBuilder().setCustomId(`embed_wiz:cancel`).setLabel('Cancel ❌').setStyle(ButtonStyle.Danger));
    const actionRows = [];
    switch (page) {
        case 1:
            ...
            actionRows.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('embed_wiz:modal:basic').setLabel('Edit Basic Info').setStyle(ButtonStyle.Secondary)));
            break;
        case 2:
            ...
            actionRows.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('embed_wiz:modal:media').setLabel('Edit Media URLs').setStyle(ButtonStyle.Secondary)));
            break;
        case 3:
            ...
            actionRows.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('embed_wiz:modal:color').setLabel('Enter HEX Color').setStyle(ButtonStyle.Secondary)));
            break;
        case 4:
            desc = `### Page 4: Author Parameters\n` +
                ...
                `Press the button below to edit these parameters.`;
            actionRows.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('embed_wiz:modal:author').setLabel('Edit Author Info').setStyle(ButtonStyle.Secondary)));
            break;
        case 5:
            desc = `### Page 5: Custom Action Buttons\n` +
                ...
                (session.buttons.length > 0 ? session.buttons.map((b, i) => `  \`${i + 1}.\` **${b.label}** (${b.style === 5 ? 'Link' : 'Action'})`).join('\n') : '  _No buttons defined._') +
            ...
            actionRows.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('embed_wiz:clear_buttons').setLabel('Clear Buttons').setStyle(ButtonStyle.Danger).setDisabled(session.buttons.length === 0)));
            break;
        case 6:
            ...
            actionRows.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('embed_wiz:modal:select_menu').setLabel('Set Select Menu').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId('embed_wiz:clear_select').setLabel('Clear Menu').setStyle(ButtonStyle.Danger).setDisabled(!session.selectMenu)));
            break;
        case 7:
            ...
            actionRows.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('embed_wiz:save').setLabel('Save Draft ✅').setStyle(ButtonStyle.Success).setDisabled(!session.name), new ButtonBuilder().setCustomId('embed_wiz:test_preview').setLabel('Test Preview 👁️').setStyle(ButtonStyle.Secondary)));
            break;
    }
...
export function buildFinalEmbedPayload(session) {
...
export const embedCommand = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub => sub.setName('create').setDescription('Open the premium interactive wizard'))
    ...
    async execute(interaction) {
        ...
        if (sub === 'create') {
            ...
            const actionSelect = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
            ...
            const confirmContainer = ComponentsV2.baseContainer(ComponentsV2.Accents.danger)
                ...
                .addActionRowComponents(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`embed_act_del:confirm:${name}`).setLabel('Confirm Delete').setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId(`embed_act_del:cancel`).setLabel('Cancel').setStyle(ButtonStyle.Secondary)));
            await interaction.editReply({
            ...
            if (parent.select_menu && parent.select_menu.options) {
                parent.select_menu.options.forEach((o) => {
            ...
            const settingsModalBtn = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('embed_settings:edit').setLabel('Edit Default Configs').setStyle(ButtonStyle.Primary));
            const container = ComponentsV2.baseContainer(ComponentsV2.Accents.primary)
                .addTextDisplayComponents(ComponentsV2.text(`# ⚙️ Embed Settings\nConfigure default values applied to new embeds.\n\n` +
    ...
    async handleButton(interaction) {
        if (!interaction.customId.startsWith('embed_wiz:') && !interaction.customId.startsWith('embed_act_del:') && !interaction.customId.startsWith('embed_settings:') && !interaction.customId.startsWith('embed_edit:'))
            return;
        const key = getSessionKey(interaction.user.id, interaction.guildId);
        if (interaction.customId.startsWith('embed_edit:')) {
            ...
            if (action === 'back_to_dashboard') {
                ...
                modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('placeholder').setLabel('Menu Placeholder').setPlaceholder('Select a category...').setValue(session.selectMenu?.placeholder || '').setStyle(TextInputStyle.Short).setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('options').setLabel('Options (CSV: Label:Value,Label:Value)').setPlaceholder('Rules:rules,FAQ:faq,Support:support').setValue(session.selectMenu?.options ? session.selectMenu.options.map((o) => `${o.label}:${o.value.replace('embed_link:', '')}`).join(',') : '').setStyle(TextInputStyle.Paragraph).setRequired(true)));
                await interaction.showModal(modal);
            }
            else if (action === 'save') {
                try {
        ...
        if (interaction.customId.startsWith('embed_wiz:')) {
            ...
            if (action === 'prev') {
                ...
                if (subModal === 'basic') {
                    ...
                    modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('name').setLabel('Embed Name (Unique Key)').setPlaceholder('rules').setValue(session.name).setStyle(TextInputStyle.Short).setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('title').setLabel('Embed Title').setPlaceholder('Server Rules').setValue(session.title).setStyle(TextInputStyle.Short).setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('description').setLabel('Embed Description').setPlaceholder('Enter description text...').setValue(session.description).setStyle(TextInputStyle.Paragraph).setRequired(false)));
                    await interaction.showModal(modal);
                }
                    ...
                    modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('thumbnailUrl').setLabel('Thumbnail Image URL').setPlaceholder('https://example.com/logo.png').setValue(session.thumbnailUrl).setStyle(TextInputStyle.Short).setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('imageUrl').setLabel('Main Image URL').setPlaceholder('https://example.com/banner.png').setValue(session.imageUrl).setStyle(TextInputStyle.Short).setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('footerText').setLabel('Footer Text').setPlaceholder('Victus Cloud hosting').setValue(session.footerText).setStyle(TextInputStyle.Short).setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('footerIconUrl').setLabel('Footer Icon URL').setPlaceholder('https://example.com/icon.png').setValue(session.footerIconUrl).setStyle(TextInputStyle.Short).setRequired(false)));
                    await interaction.showModal(modal);
                }
                else if (subModal === 'color') {
                    const modal = new ModalBuilder().setCustomId(`embed_wiz_modal:color:${Date.now()}`).setTitle('Page 3: HEX Color');
                    modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('color').setLabel('Custom Color HEX').setPlaceholder('#6366f1').setValue(session.color).setStyle(TextInputStyle.Short).setRequired(true)));
                    await interaction.showModal(modal);
                }
                    ...
                    modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('authorName').setLabel('Author Name').setPlaceholder('Victus Support Guild').setValue(session.authorName).setStyle(TextInputStyle.Short).setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('authorIconUrl').setLabel('Author Icon URL').setPlaceholder('https://example.com/author.png').setValue(session.authorIconUrl).setStyle(TextInputStyle.Short).setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('authorUrl').setLabel('Author Link URL').setPlaceholder('https://victuscloud.com').setValue(session.authorUrl).setStyle(TextInputStyle.Short).setRequired(false)));
                    await interaction.showModal(modal);
                }
            ...
            modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('color').setLabel('Default Color HEX').setPlaceholder('#6366f1').setValue(settings?.default_color || '').setStyle(TextInputStyle.Short).setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('footer').setLabel('Default Footer Text').setPlaceholder('Victus Cloud').setValue(settings?.default_footer || '').setStyle(TextInputStyle.Short).setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('author').setLabel('Default Author Name').setPlaceholder('Victus Cloud').setValue(settings?.default_author || '').setStyle(TextInputStyle.Short).setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('thumbnail').setLabel('Default Thumbnail URL').setPlaceholder('https://victuscloud.com/logo.png').setValue(settings?.default_thumbnail || '').setStyle(TextInputStyle.Short).setRequired(false)));
            await interaction.showModal(modal);
        }
    },
    async handleSelectMenu(interaction) {
        if (!interaction.customId.startsWith('embed_wiz:') && !interaction.customId.startsWith('embed_list:') && !interaction.customId.startsWith('embed_link:') && !interaction.customId.startsWith('embed_edit:'))
            return;
        const key = getSessionKey(interaction.user.id, interaction.guildId);
        if (interaction.customId.startsWith('embed_edit:')) {
            if (interaction.customId === 'embed_edit:select_embed') {
                ...
                modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('label').setLabel('Button Label').setPlaceholder('Click Here').setStyle(TextInputStyle.Short).setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('emoji').setLabel('Emoji Name/Icon (Optional)').setPlaceholder('🔥').setStyle(TextInputStyle.Short).setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('url').setLabel(style === 5 ? 'URL Link (Link style requires URL)' : 'Linked Child Embed Name / CustomId').setPlaceholder(style === 5 ? 'https://example.com' : 'support').setStyle(TextInputStyle.Short).setRequired(style === 5)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('ephemeral').setLabel('Load Embed Ephemerally? (yes/no)').setPlaceholder('no').setStyle(TextInputStyle.Short).setRequired(false)));
                await interaction.showModal(modal);
                return;
            }
            const session = editors.get(key);
            ...
            if (interaction.customId === 'embed_edit:select_color_preset') {
                ...
                if (choice === 'field:color') {
                    ...
                    modal.addComponents(new ActionRowBuilder().addComponents(nameInput), new ActionRowBuilder().addComponents(iconInput), new ActionRowBuilder().addComponents(urlInput));
                    await interaction.showModal(modal);
                }
                    ...
                    modal.addComponents(new ActionRowBuilder().addComponents(thumbInput), new ActionRowBuilder().addComponents(imgInput));
                    await interaction.showModal(modal);
                }
        ...
        if (interaction.customId === 'embed_wiz:select:color') {
            ...
            const actionContainer = ComponentsV2.baseContainer(ComponentsV2.Accents.primary)
                ...
                .addActionRowComponents(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`embed_wiz_list_act:publish:${name}`).setLabel('Publish to Channel').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId(`embed_wiz_list_act:duplicate:${name}`).setLabel('Duplicate').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(`embed_act_del:confirm:${name}`).setLabel('Delete').setStyle(ButtonStyle.Danger)));
            await interaction.update({
    ...
    async handleModal(interaction) {
        if (!interaction.customId.startsWith('embed_wiz_modal:') && !interaction.customId.startsWith('embed_settings_modal:') && !interaction.customId.startsWith('embed_edit_modal:'))
            return;
        const key = getSessionKey(interaction.user.id, interaction.guildId);
...
export const embedListActionButtons = {
    data: new SlashCommandBuilder().setName('_embed_act_internal').setDescription('internal'),
    async execute() { },
    async handleButton(interaction) {
        if (!interaction.customId.startsWith('embed_wiz_list_act:'))
            return;
        const name = interaction.customId.split(':')[2];
        const action = interaction.customId.split(':')[1];
        if (action === 'publish') {
            ...
            const channelSelect = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
    ...
    async handleSelectMenu(interaction) {
        if (!interaction.customId.startsWith('embed_pub_target:'))
            return;
...
export const embedLinksRouter = {
    ...
    async execute() { },
    async handleButton(interaction) {
        if (!interaction.customId.startsWith('embed_link:') && !interaction.customId.startsWith('embed_action:'))
            return;
    ...
    async handleSelectMenu(interaction) {
        ...
        if (!interaction.customId.startsWith('embed_select:') && !interaction.customId.startsWith('embed_link:'))
            return;
