import { SlashCommandBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, MessageFlags, ContainerBuilder, } from 'discord.js';
...
export const unlinkCommand = {
    ...
    async execute(interaction) {
        ...
        if (!existingLink) {
            const container = ComponentsV2.infoContainer('Not Linked', 'Your Discord account is not linked to any Victus Cloud account.\n\n' +
        ...
        const container = new ContainerBuilder()
            ...
            .addTextDisplayComponents(ComponentsV2.text(`# ⚠️ Confirm Unlink\n\n` +
    ...
    async handleButton(interaction) {
        if (!interaction.customId.startsWith('unlink_'))
            return;
        if (interaction.customId === 'unlink_cancel') {
            const container = ComponentsV2.infoContainer('Cancelled', 'Your account remains linked to Victus Cloud.');
        ...
        if (interaction.customId === 'unlink_confirm') {
            ...
            if (success) {
                ...
                if (interaction.member && 'roles' in interaction.member) {
                    await removeEntitlementRoles(interaction.member);
                }
                const container = ComponentsV2.successContainer('Account Unlinked', 'Your Discord account has been unlinked from Victus Cloud.\n\n' +
                ...
                if (interaction.guildId) {
                    await sendAuditLog(interaction.client, interaction.guildId, 'Account Unlinked', `👤 **User:** ${interaction.user.tag} (\`${interaction.user.id}\`)\n` +
                        `🔓 **Action:** Manual Unlink via command`, ComponentsV2.Accents.warning);
                }
                logger.info(`Account unlinked: ${interaction.user.tag} (${interaction.user.id})`);
            }
            else {
                const container = ComponentsV2.errorContainer('Error', 'Failed to unlink account. Please try again later.');
                await interaction.editReply({
                    components: [container],
                    flags: ComponentsV2.IS_COMPONENTS_V2,
                });
