import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelSelectMenuBuilder, ChannelType, MessageFlags, ModalBuilder, PermissionFlagsBits, SlashCommandBuilder, StringSelectMenuBuilder, TextInputBuilder, TextInputStyle } from 'discord.js';
import { staffAppSettings } from '../services/staffAppSettings.js';
import { ComponentsV2 } from '../embeds/componentsV2.js';
import { logger } from '../utils/logger.js';
import { supabase } from '../services/supabase.js';
const V2 = ComponentsV2.IS_COMPONENTS_V2;
const EPH = MessageFlags.Ephemeral;
function renderCategoriesDashboard(config) {
...
function renderCategorySubDashboard(cat) {
    ...
    const actionRow = new ActionRowBuilder().addComponents(new ButtonBuilder()
        ...
        .setDisabled(!cat.staffRoleId || !cat.reviewerChannelId), new ButtonBuilder()
...
function buildUnifiedApplyPanel(config) {
...
function buildSpecificApplyPanel(cat) {
    const c = ComponentsV2.baseContainer(ComponentsV2.Accents.purple);
    ...
    c.addTextDisplayComponents(ComponentsV2.text(`-# RECRUITMENT OPERATIONS\n# Apply for ${cat.displayName}!\n\n${description}`))
        .addSeparatorComponents(ComponentsV2.separator());
    const applyButton = new ActionRowBuilder().addComponents(new ButtonBuilder()
...
function buildReviewCard(submission, cat) {
    const c = ComponentsV2.baseContainer(ComponentsV2.Accents.primary);
...
function buildReviewDecidedCard(submission, cat) {
...
async function showFormModal(interaction, cat) {
...
export const staffAppCommand = {
    data: new SlashCommandBuilder()
        .setName('staff-app')
        .setDescription('Configure and deploy multiple staff recruitment application forms')
        .setDMPermission(false)
        .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
        .addSubcommand(sub => sub.setName('setup').setDescription('Open the recruitment configuration categories manager')),
    ...
    async handleButton(interaction) {
        ...
        // Candidate specific apply button action
            ...
            if (!linked) {
                await interaction.reply({
                    components: [ComponentsV2.errorContainer('Account Link Required', 'You must link your Discord account to Victus Cloud to apply for staff. Use the link panel or `/account` to link your profile first.')],
                    flags: V2 | EPH
                });
                return;
            }
    ...
    async handleModal(interaction) {
        ...
        // Candidate submitting application modal
            ...
            if (reviewerChannel && reviewerChannel.isTextBased()) {
                ...
                await reviewerChannel.send({
                    components: [card],
                    flags: V2
                }).catch((err) => logger.error('Failed to send review card to staff channel:', err));
            }
            await interaction.reply({
