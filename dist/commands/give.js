...
import { errorEmbed } from '../embeds/theme.js';
...
async function resolveTarget(interaction, rawTarget) {
    ...
    if (!discordId && interaction.guild) {
        ...
        try {
            ...
            if (members && members.size > 0) {
                ...
                for (const [, member] of members) {
                    if (member.user.username.toLowerCase() === baseName ||
        ...
        if (!discordId) {
            const cachedUser = interaction.client.users.cache.find((u) => u.username.toLowerCase() === baseName ||
                u.globalName?.toLowerCase() === baseName ||
                u.tag.toLowerCase() === usernameLower);
            if (cachedUser)
                discordId = cachedUser.id;
        }
    }
    if (!discordId) {
        return {
...
export const giveCommand = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (resolved.error) {
                await interaction.editReply({
                    embeds: [errorEmbed('Target not found', resolved.error)],
                });
                return;
            }
            ...
            if (!finalEmail && finalUserId) {
                ...
                if (profile?.email)
                    finalEmail = String(profile.email).toLowerCase();
            }
            if (!finalEmail && !finalUserId) {
                await interaction.editReply({
            ...
            try {
                ...
                if (!emailForPaymenter && finalUserId) {
                    const profile = await supabase.getUserProfile(finalUserId).catch(() => null);
                    if (profile?.email)
                        emailForPaymenter = String(profile.email).toLowerCase();
                }
                ...
                newBalance = await supabase.mutatePaymenterCoins(emailForPaymenter, amt, 'admin_give', reference, description);
            ...
            if (!success || newBalance === null) {
                await interaction.editReply({
                    embeds: [
                        errorEmbed('Failed to give coins', errorMsg
            ...
            // Log audit
            await supabase.logAudit(interaction.user.id, interaction.user.tag, 'admin_give_coins', 'paymenter_user', String(finalUserId || finalEmail || rawTarget), {
                target: rawTarget,
                resolved,
                amount: amt,
                newBalance,
            ...
            const targetDiscordId = resolved.discordId;
            if (targetDiscordId) {
                try {
                    ...
                    if (targetUser) {
                        const dmEmbed = new EmbedBuilder()
                            ...
                            .setDescription(`You just received **${amt.toLocaleString()} coins** from the Victus Cloud staff!\n\n` +
                            `**New balance:** **${Number(newBalance).toLocaleString()} coins**\n\n` +
