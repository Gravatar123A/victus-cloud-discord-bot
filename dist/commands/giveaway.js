import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelType, MessageFlags, ModalBuilder, PermissionFlagsBits, SlashCommandBuilder, TextInputBuilder, TextInputStyle } from 'discord.js';
...
function renderGiveawayWizard(session) {
    const page = session.page;
    const c = ComponentsV2.baseContainer(ComponentsV2.Accents.purple);
    let title = `Giveaway Builder • Page ${page}/3`;
    let desc = '';
    const navRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`giveaway_wiz:prev:${page}`).setLabel('⬅️ Back').setStyle(ButtonStyle.Secondary).setDisabled(page === 1), new ButtonBuilder().setCustomId(`giveaway_wiz:next:${page}`).setLabel('Next ➡️').setStyle(ButtonStyle.Primary).setDisabled(page === 3), new ButtonBuilder().setCustomId(`giveaway_wiz:cancel`).setLabel('Cancel ❌').setStyle(ButtonStyle.Danger));
    const actionRows = [];
    switch (page) {
        case 1:
            ...
            actionRows.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('giveaway_wiz:modal:basic').setLabel('Edit Basic Info').setStyle(ButtonStyle.Secondary)));
            break;
        case 2:
            ...
            actionRows.push(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('giveaway_wiz:modal:reqs').setLabel('Edit Requirements').setStyle(ButtonStyle.Secondary)));
            break;
        case 3:
            ...
            const channelSelect = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('giveaway_wiz:launch').setLabel('Launch Giveaway 🎉').setStyle(ButtonStyle.Success).setDisabled(!session.prize || !session.durationMs));
...
function buildGiveawayCard(giveaway, participantCount) {
    ...
    if (isEnded) {
        ...
        if (rId)
            reqs.push(`· Must have role: <@&${rId}>`);
        if (lvl > 0)
            reqs.push(`· Must be level: **\`${lvl}\`** or above`);
        if (invs > 0)
            reqs.push(`· Must have invited: **\`${invs}\`** or more members`);
        if (bst)
            reqs.push(`· Must be a Server Booster`);
        if (reqs.length > 0) {
    ...
    c.addTextDisplayComponents(ComponentsV2.text(body))
        .addSeparatorComponents(ComponentsV2.separator());
    if (!isEnded) {
        c.addActionRowComponents(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`giveaway:enter:${giveaway.id}`).setLabel('Participate 🎉').setStyle(ButtonStyle.Success).setDisabled(isPaused)));
    }
    return c;
}
export const giveawayCommand = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub => sub.setName('create').setDescription('Open the premium interactive creation wizard'))
        .addSubcommand(sub => sub.setName('list').setDescription('View all active giveaways'))
        .addSubcommand(sub => sub.setName('end').setDescription('Halt a giveaway and select winners immediately')
        ...
        .addStringOption(o => o.setName('id').setDescription('Giveaway ID (UUID)').setRequired(true)))
        .addSubcommand(sub => sub.setName('pause').setDescription('Pause ends_at timer of a giveaway')
        .addStringOption(o => o.setName('id').setDescription('Giveaway ID (UUID)').setRequired(true)))
        .addSubcommand(sub => sub.setName('resume').setDescription('Resume ends_at timer of a giveaway')
        .addStringOption(o => o.setName('id').setDescription('Giveaway ID (UUID)').setRequired(true)))
        .addSubcommand(sub => sub.setName('delete').setDescription('Delete a giveaway')
        .addStringOption(o => o.setName('id').setDescription('Giveaway ID (UUID)').setRequired(true))),
        ...
        if (sub === 'create') {
            ...
            if (channel && channel.type === ChannelType.GuildText) {
                const rerollContainer = ComponentsV2.baseContainer(ComponentsV2.Accents.purple)
                    .addTextDisplayComponents(ComponentsV2.text(`-# 🎁 GIVEAWAY REROLL • #${id.slice(0, 8)}\n` +
                    `# Reroll Complete! 🏆\n\n` +
        ...
        if (interaction.customId.startsWith('giveaway_wiz:')) {
            ...
            if (action === 'prev') {
                ...
                if (target === 'basic') {
                    ...
                    modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('prize').setLabel('Prize Item').setPlaceholder('Steam Key / Nitro').setValue(session.prize).setStyle(TextInputStyle.Short).setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('duration').setLabel('Duration (e.g. 10m, 1h, 2d)').setPlaceholder('1h').setValue(session.durationText).setStyle(TextInputStyle.Short).setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('winners').setLabel('Winners Slots Count').setPlaceholder('1').setValue(String(session.winnersCount)).setStyle(TextInputStyle.Short).setRequired(true)));
                    await interaction.showModal(modal);
                }
                else if (target === 'reqs') {
                    const modal = new ModalBuilder().setCustomId('giveaway_wiz_modal:reqs').setTitle('Page 2: Participation Filters');
                    modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('roleId').setLabel('Required Role ID (Optional)').setPlaceholder('827364857293847582').setValue(session.reqRoleId).setStyle(TextInputStyle.Short).setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('level').setLabel('Minimum Level Required').setPlaceholder('5').setValue(String(session.reqLevel)).setStyle(TextInputStyle.Short).setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('invites').setLabel('Minimum Invite Count Required').setPlaceholder('3').setValue(String(session.reqInvites)).setStyle(TextInputStyle.Short).setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('booster').setLabel('Require Server Booster (true/false)').setPlaceholder('false').setValue(String(session.reqBooster)).setStyle(TextInputStyle.Short).setRequired(false)));
                    await interaction.showModal(modal);
                }
            }
            else if (action === 'launch') {
                const modal = new ModalBuilder().setCustomId('giveaway_wiz_modal:launch_channel').setTitle('Page 3: Launch Target Channel');
                modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('channelNameOrId').setLabel('Target Channel Name / ID').setPlaceholder('giveaways').setStyle(TextInputStyle.Short).setRequired(true)));
                await interaction.showModal(modal);
            }
        }
            ...
            if (!linked) {
                await interaction.reply({
                    components: [ComponentsV2.errorContainer('Account Not Linked', 'You must link your Discord account to Victus Cloud to participate in giveaways. Use the link panel or `/account` to link your profile.')],
                    flags: V2 | EPH
                });
                return;
            }
    ...
    async handleModal(interaction) {
        if (!interaction.customId.startsWith('giveaway_wiz_modal:'))
            return;
        const key = getSessionKey(interaction.user.id, interaction.guildId);
        const session = wizardSessions.get(key);
        if (!session) {
        ...
        if (modalType === 'basic') {
            ...
            try {
                ...
                const dbGiveaway = await supabase.createGiveaway(interaction.guildId, channel.id, message.id, session.prize, session.durationText, session.winnersCount, new Date(endsAt), interaction.user.id, tempGiveaway.requirements, []);
                if (!dbGiveaway) {
...
async function endGiveaway(client, giveaway) {
    ...
    if (picked.length > 0) {
        const winContainer = ComponentsV2.baseContainer(ComponentsV2.Accents.purple)
            .addTextDisplayComponents(ComponentsV2.text(`-# 🎁 GIVEAWAY CONCLUDED • #${giveaway.id.slice(0, 8)}\n` +
            `# Congratulations! 🏆\n\n` +
