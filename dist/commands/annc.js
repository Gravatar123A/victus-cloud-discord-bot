 ...
 */
import { SlashCommandBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder, ChannelSelectMenuBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, ContainerBuilder, TextDisplayBuilder, SeparatorBuilder, MediaGalleryBuilder, MediaGalleryItemBuilder, ThumbnailBuilder, ChannelType, MessageFlags, SectionBuilder, } from 'discord.js';
...
function renderSetupDashboard(channels) {
    ...
    container.addTextDisplayComponents(new TextDisplayBuilder().setContent(body)).addSeparatorComponents(new SeparatorBuilder().setDivider(true));
...
function renderDashboard(draft) {
    const container = new ContainerBuilder();
    ...
    const btnRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('annc:edit_text').setLabel('Edit Text').setStyle(ButtonStyle.Secondary).setEmoji('📝'), new ButtonBuilder().setCustomId('annc:edit_design').setLabel('Edit Design').setStyle(ButtonStyle.Secondary).setEmoji('🎨'), new ButtonBuilder().setCustomId('annc:preview').setLabel('Preview').setStyle(ButtonStyle.Secondary).setEmoji('👁️'), new ButtonBuilder().setCustomId('annc:send_dispatch').setLabel('Send').setStyle(ButtonStyle.Success).setEmoji('🚀'), new ButtonBuilder().setCustomId('annc:cancel').setLabel('Cancel').setStyle(ButtonStyle.Danger).setEmoji('❌'));
    container.addActionRowComponents(btnRow);
    return container;
}
/** Construct the V2 Layout Container from the draft configuration */
function buildAnnouncementContainer(draft) {
    const container = new ContainerBuilder();
    let description = '';
    if (draft.title) {
    ...
    if (draft.imageUrl) {
        container.addMediaGalleryComponents(new MediaGalleryBuilder().addItems(new MediaGalleryItemBuilder().setURL(draft.imageUrl)));
    }
    if (draft.footerText) {
        container.addTextDisplayComponents(new TextDisplayBuilder().setContent(`-# ${draft.footerText}`));
    }
    return container;
}
export const anncCommand = {
    ...
    async execute(interaction) {
        ...
        if (subcommand === 'send') {
            ...
            if (channels.length === 0) {
                const warnContainer = ComponentsV2.warningContainer('No Channels Set', 'You have not configured any announcement channels yet.\n\nUse \`/annc setup\` to add some first!');
            ...
            if (selectOptions.length === 0) {
                const warnContainer = ComponentsV2.warningContainer('Invalid Channels', 'None of your configured announcement channels could be found in this server.');
                await interaction.editReply({ components: [warnContainer], flags: V2 });
                return;
            }
            ...
            const container = new ContainerBuilder()
                .addTextDisplayComponents(new TextDisplayBuilder().setContent(`### 📢 Announcement Sender\n` +
    ...
    async handleSelectMenu(interaction) {
        ...
        if (interaction.customId !== 'annc:select_channel')
            return;
    ...
    async handleButton(interaction) {
        if (!interaction.customId.startsWith('annc:'))
            return;
        const draft = pendingDrafts.get(interaction.user.id);
        if (!draft) {
        ...
        if (action === 'edit_design') {
            ...
            const footerInput = new TextInputBuilder()
                ...
                .setRequired(false)
                .setMaxLength(2048);
            modal.addComponents(new ActionRowBuilder().addComponents(colorInput), new ActionRowBuilder().addComponents(thumbInput), new ActionRowBuilder().addComponents(imageInput), new ActionRowBuilder().addComponents(footerInput));
            await interaction.showModal(modal);
            return;
        }
        if (action === 'preview') {
        ...
        if (action === 'send_dispatch') {
            ...
            try {
                ...
                pendingDrafts.delete(interaction.user.id);
                const success = ComponentsV2.successContainer('Announcement Dispatched', `Successfully posted the announcement to <#${channel.id}>.`);
                await interaction.update({ components: [success] });
            }
            catch (err) {
    ...
    async handleModal(interaction) {
        if (!interaction.customId.startsWith('annc:modal_'))
            return;
        const draft = pendingDrafts.get(interaction.user.id);
        if (!draft) {
            await interaction.reply({
