 ...
 */
import { SlashCommandBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, ContainerBuilder, MessageFlags, } from 'discord.js';
...
export const announceCommand = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const container = ComponentsV2.errorContainer('Error', 'Failed to process announcement command.');
    ...
    async handleButton(interaction) {
        ...
        try {
            ...
            if (customId === CUSTOM_IDS.CANCEL) {
                ...
                const container = ComponentsV2.infoContainer('Cancelled', 'Announcement has been cancelled.');
            ...
            if (customId.startsWith('announce_abort_')) {
                ...
                const container = ComponentsV2.warningContainer('Aborted', 'Announcement sending has been aborted.');
                await interaction.update({
                    components: [container],
                });
                return;
    ...
    async handleSelectMenu(interaction) {
        ...
        try {
            ...
            if (customId === CUSTOM_IDS.TARGET_SELECT) {
                const pending = pendingAnnouncements.get(interaction.user.id);
                pending.target = interaction.values[0];
            ...
            if (customId === CUSTOM_IDS.CATEGORY_SELECT) {
                const pending = pendingAnnouncements.get(interaction.user.id);
                pending.dmCategory = interaction.values[0];
                await showPreview(interaction);
                return;
            }
        }
...
async function handleModalSubmit(interaction) {
    ...
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(ComponentsV2.text(`# 📢 Create Announcement\n\n` +
        `**Title:** ${title}\n\n` +
        `Select the announcement type:\n` +
        `━━━━━━━━━━━━━━━━━━`));
    const typeSelect = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
        .setCustomId(CUSTOM_IDS.TYPE_SELECT)
        .setPlaceholder('Select type...')
        .addOptions(new StringSelectMenuOptionBuilder().setLabel('Info').setValue('info').setEmoji('ℹ️').setDescription('General information'), new StringSelectMenuOptionBuilder().setLabel('Warning').setValue('warning').setEmoji('⚠️').setDescription('Warning notice'), new StringSelectMenuOptionBuilder().setLabel('Success').setValue('success').setEmoji('✅').setDescription('Positive news'), new StringSelectMenuOptionBuilder().setLabel('Error').setValue('error').setEmoji('❌').setDescription('Critical alert')));
...
async function showTargetSelect(interaction) {
    const pending = pendingAnnouncements.get(interaction.user.id);
    const container = new ContainerBuilder()
        .setAccentColor(ComponentsV2.Accents.info)
        .addTextDisplayComponents(ComponentsV2.text(`# 📢 Create Announcement\n\n` +
        ...
        `━━━━━━━━━━━━━━━━━━`));
    const targetSelect = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
        .setCustomId(CUSTOM_IDS.TARGET_SELECT)
        .setPlaceholder('Select target...')
        .addOptions(new StringSelectMenuOptionBuilder().setLabel('Channel Only').setValue('channel').setEmoji('💬').setDescription('Post in current channel'), new StringSelectMenuOptionBuilder().setLabel('DM Only').setValue('dm').setEmoji('📬').setDescription('Send to opted-in users via DM'), new StringSelectMenuOptionBuilder().setLabel('Both').setValue('both').setEmoji('📡').setDescription('Channel + DM to opted-in users')));
...
async function showCategorySelect(interaction) {
    ...
    const categorySelect = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
        .setCustomId(CUSTOM_IDS.CATEGORY_SELECT)
        .setPlaceholder('Select DM category...')
        .addOptions(new StringSelectMenuOptionBuilder().setLabel('Maintenance').setValue('maintenance').setEmoji('🔧'), new StringSelectMenuOptionBuilder().setLabel('Billing').setValue('billing').setEmoji('💳'), new StringSelectMenuOptionBuilder().setLabel('Security').setValue('security').setEmoji('🔐'), new StringSelectMenuOptionBuilder().setLabel('Promotions').setValue('promotions').setEmoji('🎁')));
...
async function showPreview(interaction) {
    ...
    const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(ComponentsV2.text(`# 📢 Announcement Preview\n\n` +
...
async function handleConfirmSend(interaction) {
    const pending = pendingAnnouncements.get(interaction.user.id);
    if (!pending) {
        const container = ComponentsV2.errorContainer('Session Expired', 'Please start the announcement creation again.');
        await interaction.update({
            components: [container],
        });
        return;
    ...
    if (!announcement) {
        const container = ComponentsV2.errorContainer('Error', 'Failed to save announcement. Please try again.');
        await interaction.editReply({
            components: [container],
            flags: ComponentsV2.IS_COMPONENTS_V2,
        });
    ...
    // Clean up
    pendingAnnouncements.delete(interaction.user.id);
    const container = ComponentsV2.successContainer('Announcement Sent!', `Your announcement has been sent.\n\n` +
...
async function handleHistory(interaction) {
    ...
    if (announcements.length === 0) {
        const container = ComponentsV2.infoContainer('No Announcements', 'No announcements have been sent yet.');
        await interaction.editReply({
            components: [container],
            flags: ComponentsV2.IS_COMPONENTS_V2,
        });
...
async function handleAbort(interaction) {
    ...
    const job = activeSends.get(announcementId);
    if (!job) {
        const container = ComponentsV2.errorContainer('Not Found', 'No active sending job found for that announcement ID.');
        await interaction.editReply({
            components: [container],
            flags: ComponentsV2.IS_COMPONENTS_V2,
        });
    ...
    const container = ComponentsV2.successContainer('Aborted', `Announcement sending has been aborted.\n\n` +
        `**Sent:** ${job.sent}\n` +
        `**Failed:** ${job.failed}\n` +
...
// ============================================
async function sendDMsAsync(client, announcementId, pending, interaction) {
    const optedIn = await supabase.getUsersOptedInForDM(pending.dmCategory);
    if (optedIn.length === 0) {
...
// Utility Functions
// ============================================
function createAnnouncementEmbed(pending) {
    return new ContainerBuilder()
        .setAccentColor(getTypeAccent(pending.type))
        .addTextDisplayComponents(ComponentsV2.text(`# ${getTypeEmoji(pending.type)} ${pending.title}\n\n` +
        `${pending.content}\n\n` +
        `━━━━━━━━━━━━━━━━━━\n` +
