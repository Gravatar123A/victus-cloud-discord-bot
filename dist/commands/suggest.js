...
function buildSuggestionCard(suggestion, up, down) {
    const total = up + down;
    ...
    // User buttons row
    const userRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`suggest:upvote:${suggestion.id}`).setLabel('Upvote').setStyle(ButtonStyle.Success).setDisabled(suggestion.locked), new ButtonBuilder().setCustomId(`suggest:downvote:${suggestion.id}`).setLabel('Downvote').setStyle(ButtonStyle.Danger).setDisabled(suggestion.locked), new ButtonBuilder().setCustomId(`suggest:votes:${suggestion.id}`).setLabel('Voters Log').setStyle(ButtonStyle.Secondary));
    c.addActionRowComponents(userRow);
    // Mod buttons row
    const modRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`suggest_mod:approve:${suggestion.id}`).setLabel('Approve').setStyle(ButtonStyle.Success).setDisabled(suggestion.status === 'approved'), new ButtonBuilder().setCustomId(`suggest_mod:deny:${suggestion.id}`).setLabel('Deny').setStyle(ButtonStyle.Danger).setDisabled(suggestion.status === 'denied'), new ButtonBuilder().setCustomId(`suggest_mod:implement:${suggestion.id}`).setLabel('Implement').setStyle(ButtonStyle.Primary).setDisabled(suggestion.status === 'implemented'), new ButtonBuilder().setCustomId(`suggest_mod:lock:${suggestion.id}`).setLabel(suggestion.locked ? 'Unlock' : 'Lock').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(`suggest_mod:delete:${suggestion.id}`).setLabel('Delete').setStyle(ButtonStyle.Danger));
...
export const suggestCommand = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const tempSuggestion = await supabase.createSuggestion(interaction.guildId, channelId, 'temp', interaction.user.id, interaction.user.tag, title, content);
            if (!tempSuggestion) {
...
export const suggestionCommand = {
    ...
    async handleButton(interaction) {
        if (!interaction.customId.startsWith('suggest:') && !interaction.customId.startsWith('suggest_mod:'))
            return;
