import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelSelectMenuBuilder, ChannelType, MessageFlags, PermissionFlagsBits, SlashCommandBuilder, StringSelectMenuBuilder } from 'discord.js';
import { serverStatsSettings } from '../services/serverStatsSettings.js';
...
function renderStatsDashboard(config) {
    ...
    const statsSelect = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
...
export async function updateServerStats(guild) {
    try {
        ...
        const statsMap = {
            ...
            online: `🟢 Online: ${guild.members.cache.filter((m) => m.presence && m.presence.status !== 'offline').size.toLocaleString()}`,
