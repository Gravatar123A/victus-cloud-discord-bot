...
export const resourceApplyCommand = {
    ...
    async handleSelectMenu(interaction) {
        ...
        if (listing.applied) {
            await interaction.update({
                content: null,
                components: [
                    ComponentsV2.cleanContainer(ComponentsV2.Accents.warning, 'Already Applied', 'This resource listing has already been submitted for staff review.', 'DUPLICATE APPLICATION'),
                ],
            });
            return;
        }
        ...
        const reviewContainer = ComponentsV2.cleanContainer(ComponentsV2.Accents.primary, 'Resource Reward Application', `dY"O **Submission Details**\n\n` +
            ...
            `### Staff Verification\n` +
            `Review the resource for quality and compliance. Approving this submission will automatically grant **${REWARD_COINS_AMOUNT} COINS** to <@${userId}>'s Victus Cloud account balance.`, 'STAFF REVIEW PANEL');
    ...
    async handleButton(interaction) {
        ...
        const memberRoles = member && 'roles' in member ? member.roles.cache || [] : [];
        ...
        if (!isStaff && !isAdmin) {
            await interaction.reply({
                components: [
                    ComponentsV2.cleanContainer(ComponentsV2.Accents.danger, 'Permission Denied', `Only staff members with the <@&${STAFF_ROLE_ID}> role can review and approve resource submissions.`, 'ACCESS RESTRICTED'),
                ],
                flags: MessageFlags.Ephemeral | ComponentsV2.IS_COMPONENTS_V2,
            });
            return;
        ...
        if (action === 'victus_res_staff_approve') {
            ...
            const resultContainer = ComponentsV2.cleanContainer(ComponentsV2.Accents.success, 'Resource Submission Approved', `✅ **Application Approved by <@${interaction.user.id}>.**\n\n` +
                `The resource submission from <@${applicantUserId}> was reviewed and approved by staff. **${REWARD_COINS_AMOUNT} COINS** have been credited to your Victus Cloud account.`, 'APPROVAL CONFIRMED');
            await interaction.update({
            ...
            try {
                ...
                await applicant?.send(`🎉 **Congratulations!** Your resource submission was officially approved by Victus staff!\n\n` +
        ...
        if (action === 'victus_res_staff_reject') {
            ...
            const resultContainer = ComponentsV2.cleanContainer(ComponentsV2.Accents.danger, 'Resource Submission Declined', `❌ **Application Declined by <@${interaction.user.id}>.**\n\n` +
                `The resource submission from <@${applicantUserId}> was reviewed and declined by staff.`, 'SUBMISSION DECLINED');
            await interaction.update({
            ...
            try {
                const applicant = await interaction.client.users.fetch(applicantUserId).catch(() => null);
                await applicant?.send(`⚠️ Your resource submission application was reviewed by Victus staff and declined at this time. ` +
