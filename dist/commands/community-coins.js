...
const COMMUNITY_COINS_INVITE = `https://discord.com/oauth2/authorize?client_id=${config.discord.clientId}&scope=bot&permissions=1`;
// Where publishers create their listing (Victus free panel → Community → Community Coins).
const PUBLISH_URL = config.branding.free;
export const communityCoinsCommand = {
    ...
    async execute(interaction) {
        ...
        if (!guildId) {
            await interaction.editReply({
                components: [
                    ComponentsV2.warningContainer('Run This In Your Server', 'Use `/community-coins` inside the Discord server you want to publish so I can read its **Server ID**.'),
                ],
                flags: V2,
            });
            return;
        ...
        container
            .addTextDisplayComponents(ComponentsV2.text(`-# 💠 VICTUS CLOUD CONNECTION • COMMUNITY COINS\n` +
            ...
            .setURL(COMMUNITY_COINS_INVITE), new ButtonBuilder()
            .setLabel('Open Free Panel')
            .setStyle(ButtonStyle.Link)
            .setURL(PUBLISH_URL), new ButtonBuilder()
            .setLabel('Victus Cloud')
            .setStyle(ButtonStyle.Link)
            .setURL(config.branding.website)))
            .addTextDisplayComponents(ComponentsV2.text(`-# Keep the Victus bot in your server so joins can be verified — removing it disables COINS payouts.`));
        await interaction.editReply({
