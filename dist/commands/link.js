...
export const linkCommand = {
    ...
    async execute(interaction) {
        ...
        if (existingLink) {
            const container = ComponentsV2.infoContainer('Already Linked', 'Your Discord account is already linked to a Victus Cloud account.\n\n' +
        ...
        if (!linkToken) {
            const container = ComponentsV2.errorContainer('Error', 'Failed to generate link token. Please try again later.');
            await interaction.editReply({
                components: [container],
                flags: ComponentsV2.IS_COMPONENTS_V2,
            });
        ...
        const pollInterval = setInterval(async () => {
            ...
            if (linked) {
                ...
                try {
                    ...
                    logger.warn(`Polling link COINS grant failed for ${discordId}: ${e.message}`);
                }
                // 1. Assign Role
                ...
                // 3. Send Audit Log
                if (guildId) {
                    await sendAuditLog(interaction.client, guildId, 'Account Linked (Fallback)', `👤 **User:** <@${discordId}> (\`${discordId}\`)\n` +
                        `🔗 **Status:** ${roleSuccess ? '✅ Role Assigned' : '⚠️ User not in server'}`, ComponentsV2.Accents.success);
                }
            }
