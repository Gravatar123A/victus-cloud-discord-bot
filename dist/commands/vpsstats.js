import { MessageFlags, PermissionFlagsBits, SlashCommandBuilder, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle } from 'discord.js';
...
function makeProgressBar(percent, size = 15) {
...
export const vpsStatsCommand = {
    ...
    async execute(interaction) {
        ...
        if (isPrefix) {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields({ name: '💻 CPU Usage', value: `\`${makeProgressBar(cpuUsage)}\` **${cpuUsage}%**\n› Cores: ${os.cpus().length}x\n› Load: ${loadAvg[0].toFixed(2)}` }, { name: '💾 Memory Usage', value: `\`${makeProgressBar(memPercent)}\` **${memPercent}%**\n› Used: ${(usedMem / 1024 / 1024 / 1024).toFixed(2)} / ${(totalMem / 1024 / 1024 / 1024).toFixed(2)} GB` })
                .setTimestamp();
            if (disk) {
    ...
    async handleButton(interaction) {
        if (interaction.customId !== 'vpsstats:refresh')
            return;
