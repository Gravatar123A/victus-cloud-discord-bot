import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelSelectMenuBuilder, ChannelType, MessageFlags, ModalBuilder, PermissionFlagsBits, SlashCommandBuilder, StringSelectMenuBuilder, TextInputBuilder, TextInputStyle, RoleSelectMenuBuilder } from 'discord.js';
...
function renderWelcomeDashboard(config) {
    ...
    const formatSelect = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
    ...
    const toggleRow = new ActionRowBuilder().addComponents(new ButtonBuilder()
        .setCustomId('welcome_wiz:toggle_status')
        .setLabel(config.enabled ? 'Disable System 🔴' : 'Enable System 🟢')
        .setStyle(config.enabled ? ButtonStyle.Danger : ButtonStyle.Success), new ButtonBuilder()
    ...
    const editRow = new ActionRowBuilder().addComponents(new ButtonBuilder()
        ...
        .setDisabled(config.welcomeType === 'custom_embed'), new ButtonBuilder()
        .setCustomId('welcome_wiz:modal:embed')
        .setLabel('Edit Embed Style')
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(config.welcomeType !== 'embed'), new ButtonBuilder()
...
export function formatWelcomeMessage(template, member) {
...
export async function buildWelcomePayload(config, member) {
...
export const welcomeCommand = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub => sub.setName('setup').setDescription('Open the welcome system settings wizard'))
        .addSubcommand(sub => sub.setName('test').setDescription('Send a test welcome greeting message to the configured channel')),
    ...
    async handleButton(interaction) {
        if (!interaction.customId.startsWith('welcome_wiz:'))
            return;
        const config = await welcomeSettings.get(interaction.guildId);
    ...
    async handleSelectMenu(interaction) {
        ...
        if (action === 'channel') {
            ...
            const updated = await welcomeSettings.set(interaction.guildId, { welcomeType: val });
            await interaction.update({ components: [renderWelcomeDashboard(updated)], embeds: [] });
        }
    },
    async handleModal(interaction) {
        if (!interaction.customId.startsWith('welcome_wiz_modal:'))
            return;
        ...
        if (type === 'msg') {
            ...
            await interaction.update({ components: [renderWelcomeDashboard(updated)], embeds: [], flags: V2 });
        }
        else if (type === 'embed') {
