...
export function createEmbed(options) {
    ...
    const embed = new EmbedBuilder()
        .setColor((typeof color === 'number' ? color : VICTUS_COLORS[color]));
    if (title)
        embed.setTitle(title);
    if (description)
        embed.setDescription(description);
    if (banner)
        embed.setImage(BOT_BANNER_URL);
    if (thumbnail)
        embed.setThumbnail(config.branding.logo);
    if (footer) {
        embed.setFooter({
 ...
 * Loading embed
 */
export function loadingEmbed(message = 'Processing...') {
    return createEmbed({
 ...
 */
export function notLinkedEmbed() {
    return createEmbed({
        title: '🔗 Account Not Linked',
        description: 'Your Discord account is not linked to Victus Cloud.\n\n' +
 ...
 */
export function getServerStatusColor(status) {
 ...
 * Server status emoji
 */
export function getServerStatusEmoji(status) {
    switch (status?.toLowerCase()) {
        case 'running':
            return '🟢';
        case 'starting':
 ...
 */
export function getInvoiceStatusEmoji(status) {
    switch (status?.toLowerCase()) {
        case 'paid':
            return '✅';
        case 'unpaid':
