...
export class SuggestionEmbeds {
     ...
     */
    static buildLauncher(selectedKeys = []) {
        ...
        const body = `-# 💡 VICTUS CLOUD • COMMUNITY SUGGESTION HUB\n` +
            `# Draft a Suggestion\n\n` +
        ...
        const btnRow = new ActionRowBuilder().addComponents(openModalButton, cancelBtn);
        return {
            container,
            rows: [menuRow, btnRow],
        };
     ...
     */
    static renderProgressBar(up, down) {
        const total = up + down;
        if (total === 0)
     ...
     */
    static buildSuggestionCard(suggestion, upvotes = 0, downvotes = 0, userVote) {
        ...
        const categoryBadges = cats
            .map((k) => {
            const def = SUGGESTION_CATEGORIES[k];
        ...
        let body = `-# 💡 VICTUS CLOUD • COMMUNITY PROPOSAL #${suggestion.id}\n` +
            `\`${statusTag}\`\n\n` +
            `# ${suggestion.title}\n\n` +
        ...
        if (suggestion.staff_reviewer_id || suggestion.staff_response) {
            ...
            const actionLabel = suggestion.status === 'approved'
                ? 'ACCEPTED'
        ...
        const communityRow = new ActionRowBuilder().addComponents(new ButtonBuilder()
            ...
            .setDisabled(suggestion.locked), new ButtonBuilder()
        ...
        const staffRow = new ActionRowBuilder().addComponents(new ButtonBuilder()
            .setCustomId(`suggest_mod:accept:${suggestion.id}`)
            .setLabel('Accept')
            .setEmoji('✅')
            .setStyle(ButtonStyle.Success)
            .setDisabled(suggestion.status === 'approved'), new ButtonBuilder()
            .setCustomId(`suggest_mod:reject:${suggestion.id}`)
            .setLabel('Reject')
            .setEmoji('❌')
            .setStyle(ButtonStyle.Danger)
            .setDisabled(suggestion.status === 'denied'), new ButtonBuilder()
            .setCustomId(`suggest_mod:implement:${suggestion.id}`)
            .setLabel('Implement')
            .setEmoji('🚀')
            .setStyle(ButtonStyle.Primary)
            .setDisabled(suggestion.status === 'implemented'), new ButtonBuilder()
     ...
     */
    static buildVotersCard(suggestionId, votes) {
        ...
        container.addTextDisplayComponents(ComponentsV2.text(`-# 💡 VICTUS CLOUD • VOTER AUDIT LOG\n` +
