import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ContainerBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, } from 'discord.js';
...
function navRow(ownerId, current, isAdmin = false) {
    const opts = [
    ...
    if (isAdmin)
        opts.push({ label: 'Admin Controls', value: 'admin', emoji: '🛠️', description: 'Adjust / freeze (staff)' });
...
export function cpDashboardContainer(o) {
    ...
    const c = new ContainerBuilder()
        ...
        .addTextDisplayComponents(ComponentsV2.text(`# ${tier.emoji} Victus Economy\n` +
    ...
    c.addActionRowComponents(btnRow(new ButtonBuilder().setCustomId(`econ:dash:${o.discordId}`).setLabel('Refresh').setStyle(ButtonStyle.Secondary).setEmoji('🔄'), new ButtonBuilder().setCustomId(`econ:nav2:${o.discordId}:transfer`).setLabel('Transfer').setStyle(ButtonStyle.Primary).setEmoji('💸'), new ButtonBuilder().setLabel('Open Web').setStyle(ButtonStyle.Link).setURL(`${config.branding.website}/dashboard/rank`).setEmoji('🌐')));
    return c;
}
// ── Bank ────────────────────────────────────────────────────────────────────
export function bankContainer(discordId, profile, isAdmin = false) {
    const cp = Number(profile?.total_cp ?? 0);
    const bank = Number(profile?.cp_bank ?? 0);
    const c = new ContainerBuilder()
        .setAccentColor(ComponentsV2.Accents.primary)
        .addTextDisplayComponents(ComponentsV2.text(`# 🏦 Coins Bank\n` +
    ...
    c.addActionRowComponents(btnRow(new ButtonBuilder().setCustomId(`econ:bankdep:${discordId}`).setLabel('Deposit').setStyle(ButtonStyle.Success).setEmoji('📥'), new ButtonBuilder().setCustomId(`econ:bankwd:${discordId}`).setLabel('Withdraw').setStyle(ButtonStyle.Primary).setEmoji('📤').setDisabled(bank <= 0), new ButtonBuilder().setCustomId(`econ:dash:${discordId}`).setLabel('Back').setStyle(ButtonStyle.Secondary).setEmoji('🏠')));
    return c;
}
// ── Transfer ────────────────────────────────────────────────────────────────
export function transferContainer(discordId, profile, isAdmin = false) {
    const c = new ContainerBuilder()
        .setAccentColor(ComponentsV2.Accents.primary)
        .addTextDisplayComponents(ComponentsV2.text(`# 💸 Transfer\n` +
    ...
    c.addActionRowComponents(btnRow(new ButtonBuilder().setCustomId(`econ:xfer:${discordId}:cp`).setLabel('Send Coins').setStyle(ButtonStyle.Primary).setEmoji('⭐'), new ButtonBuilder().setCustomId(`econ:xfer:${discordId}:credits`).setLabel('Send Credits').setStyle(ButtonStyle.Secondary).setEmoji('💳'), new ButtonBuilder().setCustomId(`econ:dash:${discordId}`).setLabel('Back').setStyle(ButtonStyle.Secondary).setEmoji('🏠')));
    return c;
}
// ── Convert ─────────────────────────────────────────────────────────────────
export function convertContainer(discordId, rates, profile, credits, isAdmin = false) {
    const rate = (from, to) => rates.find((r) => r.from_currency === from && r.to_currency === to);
    const cpToCredits = rate('cp', 'credits');
    const creditsToCp = rate('credits', 'cp');
    const rateLine = (r, fromLabel, toLabel) => r ? `• 1 ${fromLabel} = **${r.rate}** ${toLabel} (min ${fmt(r.min_amount)})` : `• ${fromLabel} → ${toLabel}: \`unavailable\``;
    const c = new ContainerBuilder()
        .setAccentColor(ComponentsV2.Accents.info)
        .addTextDisplayComponents(ComponentsV2.text(`# 🔁 Convert Currency\n` +
    ...
    c.addActionRowComponents(btnRow(new ButtonBuilder().setCustomId(`econ:conv:${discordId}:cp_credits`).setLabel('Coins → Credits').setStyle(ButtonStyle.Primary).setEmoji('💳').setDisabled(!cpToCredits), new ButtonBuilder().setCustomId(`econ:conv:${discordId}:credits_cp`).setLabel('Credits → Coins').setStyle(ButtonStyle.Secondary).setEmoji('⭐').setDisabled(!creditsToCp), new ButtonBuilder().setLabel('Deposit Credits').setStyle(ButtonStyle.Link).setURL(`${config.branding.billing}/account/credits`).setEmoji('💰'), new ButtonBuilder().setCustomId(`econ:dash:${discordId}`).setLabel('Back').setStyle(ButtonStyle.Secondary).setEmoji('🏠')));
    return c;
}
// ── Admin ───────────────────────────────────────────────────────────────────
export function adminContainer(discordId) {
    const c = new ContainerBuilder()
        .setAccentColor(ComponentsV2.Accents.warning)
        .addTextDisplayComponents(ComponentsV2.text(`# 🛠️ Admin Economy Controls\n` +
    ...
    c.addActionRowComponents(btnRow(new ButtonBuilder().setCustomId(`econ:adjadj:${discordId}`).setLabel('Adjust Coins').setStyle(ButtonStyle.Primary).setEmoji('⚖️'), new ButtonBuilder().setCustomId(`econ:adjfreeze:${discordId}:1`).setLabel('Freeze').setStyle(ButtonStyle.Danger).setEmoji('🧊'), new ButtonBuilder().setCustomId(`econ:adjfreeze:${discordId}:0`).setLabel('Unfreeze').setStyle(ButtonStyle.Success).setEmoji('🔥'), new ButtonBuilder().setCustomId(`econ:dash:${discordId}`).setLabel('Back').setStyle(ButtonStyle.Secondary).setEmoji('🏠')));
    return c;
}
// ── Confirmation ────────────────────────────────────────────────────────────
export function confirmContainer(discordId, token, title, summary) {
    return new ContainerBuilder()
        .setAccentColor(ComponentsV2.Accents.warning)
        .addTextDisplayComponents(ComponentsV2.text(`# ⚠️ ${title}\n${summary}\n${HR}\n-# Please confirm to continue.`))
        .addActionRowComponents(btnRow(new ButtonBuilder().setCustomId(`econ:cfm:${discordId}:${token}`).setLabel('Confirm').setStyle(ButtonStyle.Success).setEmoji('✅'), new ButtonBuilder().setCustomId(`econ:dash:${discordId}`).setLabel('Cancel').setStyle(ButtonStyle.Secondary).setEmoji('✖️')));
}
// ── Result ──────────────────────────────────────────────────────────────────
export function resultContainer(discordId, ok, title, body, isAdmin = false) {
    return new ContainerBuilder()
        ...
        .addActionRowComponents(btnRow(new ButtonBuilder().setCustomId(`econ:dash:${discordId}`).setLabel('Back to Dashboard').setStyle(ButtonStyle.Primary).setEmoji('🏠')));
}
export function leaderboardContainer(o) {
    ...
    const c = new ContainerBuilder()
        .setAccentColor(ComponentsV2.Accents.primary)
        .addTextDisplayComponents(ComponentsV2.text(`# 🏆 Coins Leaderboard\nTop contributors across Victus Cloud.\n${HR}\n${lines}\n${HR}\n` +
    ...
    c.addActionRowComponents(btnRow(new ButtonBuilder().setCustomId(`econ:lb:${o.discordId}:${Math.max(0, o.page - 1)}`).setLabel('Prev').setStyle(ButtonStyle.Secondary).setEmoji('◀️').setDisabled(o.page <= 0), new ButtonBuilder().setCustomId(`econ:dash:${o.discordId}`).setLabel('Dashboard').setStyle(ButtonStyle.Secondary).setEmoji('🏠'), new ButtonBuilder().setCustomId(`econ:lb:${o.discordId}:${o.page + 1}`).setLabel('Next').setStyle(ButtonStyle.Secondary).setEmoji('▶️').setDisabled(o.rows.length < ECONOMY_PAGE_SIZE)));
    return c;
}
...
export function historyContainer(o) {
    ...
    c.addActionRowComponents(btnRow(new ButtonBuilder().setCustomId(`econ:hist:${o.discordId}:${Math.max(0, o.page - 1)}`).setLabel('Prev').setStyle(ButtonStyle.Secondary).setEmoji('◀️').setDisabled(o.page <= 0), new ButtonBuilder().setCustomId(`econ:dash:${o.discordId}`).setLabel('Dashboard').setStyle(ButtonStyle.Secondary).setEmoji('🏠'), new ButtonBuilder().setCustomId(`econ:hist:${o.discordId}:${o.page + 1}`).setLabel('Next').setStyle(ButtonStyle.Secondary).setEmoji('▶️').setDisabled(!o.hasNext)));
    return c;
}
