...
export function panelTitle(title, eyebrow = 'COMMAND LAYER') {
    return `${brandLine(eyebrow)}\n# ${title}`;
}
function premiumContainer(accent, title, description, eyebrow, imageUrl = HERO_IMAGE) {
    const container = baseContainer(accent);
    if (imageUrl)
        container.addMediaGalleryComponents(mediaGallery(imageUrl));
    container
...
function clampPanelText(value, maxLength) {
    if (value.length <= maxLength)
        return value;
    return `${value.slice(0, maxLength - 24).trim()}... [message trimmed]`;
}
function createBrandedContainer(accent, title, description, eyebrow) {
...
export function moderationWarningContainer(title, description, guildId, otherLanguageChannelId) {
    const container = baseContainer(Accents.warning)
        .addTextDisplayComponents(text(`${panelTitle(`⚠️ ${title}`, 'SERVER LANGUAGE POLICY')}\n\n` +
        `${description}\n\n` +
...
export function cleanContainer(accent, title, description, eyebrow = 'RESOURCE VERIFICATION') {
    const container = baseContainer(accent);
    container
        .addTextDisplayComponents(text(`${panelTitle(title, eyebrow)}\n\n${description}`))
        .addSeparatorComponents(separator())
...
export function linkAccountContainer(username, avatarUrl, expiryTimestamp, linkUrl) {
    const container = premiumContainer(Accents.discord, 'Link Your Victus Cloud Account', `**Confirm the Discord account and Victus account before connecting them.**\n\n` +
        ...
        `› Confirm the connection`, 'PRIVATE LINK SESSION');
    if (avatarUrl) {
    ...
    const buttons = new ActionRowBuilder().addComponents(new ButtonBuilder()
        ...
        .setStyle(ButtonStyle.Link)
        .setURL(linkUrl), new ButtonBuilder()
        .setLabel('Create Account')
        .setStyle(ButtonStyle.Link)
        .setURL(`${config.branding.website}/discord-signup?from=bot`), new ButtonBuilder()
        .setLabel('Help')
        .setStyle(ButtonStyle.Link)
...
export function linkPanelContainer() {
    const container = premiumContainer(Accents.primary, 'Victus Cloud Account Link Panel', `**Bind Discord to Victus Cloud and unlock account-aware controls.**\n\n` +
        `### Unlocks\n` +
        ...
        `### Security\n` +
        `Each click creates a private expiring token for the user who pressed it. The final website page shows both accounts before linking.`, 'PUBLIC CONNECTION PANEL');
    const buttons = new ActionRowBuilder().addComponents(new ButtonBuilder()
...
export function adminDmContainer(subject, message, adminEmail) {
    const container = premiumContainer(Accents.primary, subject, `${message}\n\n` +
        `### Source\n` +
        `› Sent by **Victus Cloud Admin**${adminEmail ? ` (${adminEmail})` : ''}\n` +
        `› Delivery channel: Discord direct message\n` +
        `› This message was queued from the admin panel`, 'ADMIN DIRECT MESSAGE');
    return container
...
export function helpMenuContainer(username, _avatarUrl, commandCount) {
    const container = premiumContainer(Accents.primary, 'Victus Cloud Help Menu', `Welcome, **${username}**.\n\n` +
        `### Live command surface\n` +
        `› **${commandCount}** slash commands available\n` +
        `› Account linking and role sync\n` +
        `› Server, billing, services, and support workflows\n\n` +
        `Use the category selector below to open a command group.`, 'HELP CENTER');
    ...
    const buttons = new ActionRowBuilder().addComponents(new ButtonBuilder()
        .setLabel('Invite')
        .setStyle(ButtonStyle.Link)
        .setURL(INVITE_URL), new ButtonBuilder()
        .setLabel('Dashboard')
        .setStyle(ButtonStyle.Link)
        .setURL(config.branding.website), new ButtonBuilder()
        .setLabel('Support')
...
export function aiChatContainer(question, answer, model, linked) {
    const container = premiumContainer(Accents.info, 'Victus Cloud AI', `**Question**\n${clampPanelText(question, 900)}\n\n` +
        `**Answer**\n${clampPanelText(answer, 2900)}`, 'GROQ LLAMA SUPPORT');
    const buttons = new ActionRowBuilder().addComponents(new ButtonBuilder()
        .setLabel('Open Dashboard')
        .setStyle(ButtonStyle.Link)
        .setURL(config.branding.website), new ButtonBuilder()
...
export function invoiceListContainer(invoices) {
...
export function servicesListContainer(services) {
...
export function userInfoContainer(username, discordId, isLinked, profile, servers = [], history = [], creditBalance, services = [], coins) {
    ...
    if (isLinked && profile) {
        ...
        if (servers.length > 0) {
            ...
            if (servers.length > 6)
                content += `-# Showing 6 of ${servers.length}. Use Victus Cloud for the full fleet.\n`;
        }
        else {
        ...
        if (services.length > 0) {
            ...
            if (services.length > 6)
                content += `-# Showing 6 of ${services.length}. Use Victus Cloud for all services.\n`;
        }
        else {
