 ...
 */
export async function requireLinkedAccount(interaction) {
    const linkedAccount = await supabase.getLinkedAccount(interaction.user.id);
    if (!linkedAccount) {
        const container = ComponentsV2.warningContainer('Account Not Linked', 'You need to link your Discord account to Victus Cloud first.\n\n' +
 ...
 */
export async function requireLinked(interaction) {
    const linkedAccount = await supabase.getLinkedAccount(interaction.user.id);
    if (!linkedAccount) {
        const container = ComponentsV2.warningContainer('Account Not Linked', 'You need to link your Discord account to Victus Cloud first.\n\n' +
            'Use `/link` to connect your account.');
