...
export async function registerApplicationCommands(source = 'startup') {
    ...
    if (!instantGuilds.length) {
        logger.warn('⚠️ No DISCORD_GUILD_ID / DISCORD_SUPPORT_GUILD_ID set — only global sync was done, so NEW commands can take up to ~1 hour to appear. Set a guild ID for instant updates.');
    }
}
