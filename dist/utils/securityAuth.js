 ...
 */
export async function isGrav(user, client, guild) {
    const userId = user.id;
    // 1. Explicit env variable
    if (process.env.GRAV_USER_ID && process.env.GRAV_USER_ID === userId)
        return true;
    if (process.env.PRIMARY_OWNER_ID && process.env.PRIMARY_OWNER_ID === userId)
        return true;
    ...
    if (supportGuildId) {
        try {
            ...
            if (supportGuild && supportGuild.ownerId === userId)
                return true;
        }
        catch (err) {
 ...
 */
export async function canManageSecurity(user, client, guild, member) {
