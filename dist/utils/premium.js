...
export function decodeDisplayText(value, fallback = "Unknown") {
...
export function statusLabel(status) {
    ...
    if (["running", "active", "paid", "completed"].includes(normalized))
        return "Online";
    if (["starting", "pending", "processing"].includes(normalized))
        return "Warming";
    if (["stopping"].includes(normalized))
        return "Cooling";
    if (["suspended", "cancelled", "failed", "overdue"].includes(normalized))
        return "Attention";
    if (["offline", "stopped"].includes(normalized))
        return "Offline";
...
export function statusIcon(status) {
    ...
    if (["running", "active", "paid", "completed"].includes(normalized))
        return "●";
    if (["starting", "pending", "processing", "stopping"].includes(normalized))
        return "◐";
    if (["suspended", "cancelled", "failed", "overdue"].includes(normalized))
        return "◆";
    if (["offline", "stopped"].includes(normalized))
