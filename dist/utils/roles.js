...
export async function syncRankRole(client, discordId, level) {
    ...
    if (supportGuildId) {
        try {
            ...
            if (member) {
                ...
                if (removable.length)
                    await member.roles.remove(removable, 'Victus rank synchronization');
                if (desired && !member.roles.cache.has(desired))
                    await member.roles.add(desired, `Victus rank: ${tier.name}`);
            }
        }
        catch (err) {
        ...
        for (const guild of client.guilds.cache.values()) {
            ...
            if (member) {
                ...
                if (matchingRole && guild.members.me?.permissions.has('ManageRoles') && matchingRole.comparePositionTo(guild.members.me.roles.highest) < 0) {
                    const otherTierRoles = guild.roles.cache.filter((r) => Object.keys(RANK_ROLE_IDS).map((n) => n.toLowerCase()).includes(r.name.toLowerCase()) &&
                        r.id !== matchingRole.id &&
                        member.roles.cache.has(r.id));
                    if (otherTierRoles.size)
                        await member.roles.remove(otherTierRoles, 'Victus rank sync');
...
export async function syncAllRankRoles(client) {
    ...
    for (const account of accounts) {
        ...
        if (await syncRankRole(client, account.discord_id, level).catch(() => false))
            synced++;
    }
 ...
 */
async function getBotConfig(client, discordId) {
 ...
 */
export async function assignLinkedRole(client, discordId) {
 ...
 */
export async function removeLinkedRole(client, discordId) {
    const { supportGuildId, linkedRoleId } = await getBotConfig(client, discordId);
 ...
 */
export async function syncLinkedRoles(client) {
    try {
