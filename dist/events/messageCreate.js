...
const userQueueDepth = new Map();
function enqueuePerUser(userId, task) {
    const depth = userQueueDepth.get(userId) || 0;
    if (depth >= MAX_QUEUE_DEPTH)
        return false; // too many already pending; drop the overflow
...
async function getAiChannelId(guildId) {
...
function buildPromptFromMessage(message) {
    ...
    if (content)
        return content;
    if (attachments)
        return `The user sent attachments and may need support: ${attachments}`;
    return '';
}
...
async function mirrorPublicReply(message, publicReply, text) {
    if (!publicReply || !message.inGuild() || !text)
        return;
...
async function replyWithAi(message, prompt, publicReply, fallbackMessage) {
    try {
    ...
    async execute(message) {
        ...
        if (message.inGuild()) {
            const moderated = await inspectModerationMessage(message).catch((error) => {
                logger.debug(`Moderation inspection failed for ${message.id}: ${error.message}`);
                return false;
            });
            if (moderated)
                return;
            ...
            try {
                ...
                if (afkData) {
                    ...
                    const welcomeEmbed = new EmbedBuilder()
                        ...
                        .addFields({ name: 'You were AFK for', value: `**${durationStr}**`, inline: true }, { name: 'Reason', value: afkData.reason || 'AFK', inline: true });
                    const loggedMentions = afkData.mentions || [];
                    if (loggedMentions.length > 0) {
                        const mentionList = loggedMentions
                            .map((m) => `› **${m.authorTag || m.username || m.tag || m.authorName || 'Unknown User'}** in <#${m.channelId}>: [Jump to Message](https://discord.com/channels/${guildId}/${m.channelId}/${m.messageId}) (<t:${Math.floor(new Date(m.timestamp).getTime() / 1000)}:R>)`)
                            .slice(0, 10)
                            .join('\n');
            ...
            if (rawArgs.length > 0) {
                ...
                if (commandName === 'translate') {
                    ...
                    if (subAction === 'toggle') {
                        ...
                        await message.reply(updated?.enabled
                            ? `🟢 **Live Translation Resumed!** Auto-translating between **${updated.languageEmoji} ${updated.languageName}** and **🇬🇧 English**.`
                            : `⏸️ **Live Translation Paused.** Translation disabled by <@${message.author.id}>.`).catch(() => { });
                        return;
                    }
                ...
                if (commandName) {
                    ...
                    if (command) {
                        if (command.cooldown) {
                            const remaining = checkCooldown({ user: message.author }, commandName, command.cooldown);
                            if (remaining > 0) {
                                const container = ComponentsV2.warningContainer('Slow Down!', `Please wait **${remaining}** second${remaining > 1 ? 's' : ''} before using this command again.`);
                        ...
                        if (command.requiresLink) {
                            const linked = await supabase.getLinkedAccount(message.author.id).catch(() => null);
                            if (!linked) {
                                const container = ComponentsV2.warningContainer('Account Link Required', 'This command requires you to link your Discord account to Victus Cloud. Use `/link` to start.');
                                await message.reply(translateV2Components({ components: [container], flags: ComponentsV2.IS_COMPONENTS_V2 })).catch(() => { });
                                return;
                            }
                        }
                        ...
                        try {
                            ...
                            await command.execute(prefixInteraction);
                        }
                        catch (error) {
                    ...
                    if (message.inGuild()) {
                        ...
                        if (customCmd && customCmd.enabled) {
                            if (customCmd.cooldown > 0) {
                                const remaining = checkCooldown({ user: message.author }, `custom:${commandName}`, customCmd.cooldown);
                                if (remaining > 0) {
                                    const container = ComponentsV2.warningContainer('Slow Down!', `Please wait **${remaining}** second${remaining > 1 ? 's' : ''} before using this command again.`);
                                    await message.reply(translateV2Components({ components: [container], flags: ComponentsV2.IS_COMPONENTS_V2 })).catch(() => { });
                                    return;
                            ...
                            if (customCmd.permissions && customCmd.permissions.length > 0) {
                                ...
                                if (!hasRole && !isAdmin) {
                                    const container = ComponentsV2.errorContainer('Permission Denied', 'You do not have the required roles to run this custom command.');
                                    await message.reply(translateV2Components({ components: [container], flags: ComponentsV2.IS_COMPONENTS_V2 })).catch(() => { });
                                    return;
                                }
                            }
        ...
        if (!isMentioned && !summoned && !isAiChannel)
            return;
