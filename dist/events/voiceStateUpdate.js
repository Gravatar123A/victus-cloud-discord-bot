...
export function restoreVoiceXpSessions(client) {
    clientRef = client;
    for (const guild of client.guilds.cache.values()) {
        for (const state of guild.voiceStates.cache.values()) {
            if (state.member?.user.bot || !state.channel)
                continue;
...
export const voiceStateUpdateEvent = {
    ...
    async execute(oldState, newState) {
                ...
                if (j2cConfig.enabled && j2cConfig.channelId) {
                    ...
                    if (newState.channelId === j2cConfig.channelId && member && !member.user.bot) {
                        ...
                        try {
                            ...
                            dmContainer.addTextDisplayComponents(ComponentsV2.text(`# 🎉 Voice Channel Created!\n\n` +
        ...
        if (config.economy.xpPerVoiceMinute <= 0) {
            if (newState.guild)
                stopSession(newState.guild.id, newState.id);
            return;
        }
        const member = newState.member ?? oldState.member;
        if (member?.user.bot)
