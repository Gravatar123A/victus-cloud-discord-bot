 ...
 * the support guild.
 */
async function handleInviteAttribution(member) {
    ...
    try {
        ...
        logger.warn(`Invite attribution: cannot fetch invites for guild ${member.guild.id} (bot needs Manage Server): ${err.message}`);
        return;
    }
    ...
    for (const invite of fetched.values()) {
        ...
        if (now > prev)
            grew.push({ code: invite.code, inviterId });
    }
            ...
            if (auditConfig.enabled && auditConfig.channelId && auditConfig.events.includes('member_join')) {
                ...
                if (logChannel && logChannel.isTextBased()) {
                    ...
                    const embed = new EmbedBuilder()
                        ...
                        .addFields({ name: 'Account Creation Date', value: accountAge })
                        .setTimestamp();
