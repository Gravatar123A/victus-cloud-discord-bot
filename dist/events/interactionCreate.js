    ...
    async execute(interaction) {
        ...
        if (interaction.isChatInputCommand()) {
            ...
            if (command.adminOnly) {
                ...
                if (!isStaff) {
                    const container = ComponentsV2.errorContainer('Permission Denied', 'This command controls Victus Cloud infrastructure and is strictly restricted to verified platform administrators.');
            ...
            try {
                ...
                if (error.errors)
                    logger.error('Validation details:', JSON.stringify(error.errors, null, 2));
                if (error?.code)
                    logger.error(`Discord error code: ${error.code} | HTTP: ${error?.httpStatus || error?.status || 'n/a'}`);
                ...
                const container = ComponentsV2.errorContainer('Command Error', `An error occurred while executing \`/${interaction.commandName}\`. ${hint}\n\nWe are using the standard interface for now. If this persists, contact support with the command name and time.`);
                const replyOptions = {
                    components: [container],
                    flags: ComponentsV2.IS_COMPONENTS_V2 | MessageFlags.Ephemeral,
                };
