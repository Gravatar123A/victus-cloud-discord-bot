...
export function isActiveFreeServer(resource, now = new Date()) {
...
export function isActivePaidService(resource, now = new Date()) {
...
export function resolveEntitlementRole(hasPaidServer, hasFreeServer) {
    if (hasPaidServer)
        return 'paid';
    if (hasFreeServer)
        return 'free';
