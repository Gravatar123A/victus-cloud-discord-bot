 ...
 */
async function fetchWithTimeout(url, timeoutMs = DEFAULT_FETCH_TIMEOUT, customHeaders = {}) {
 ...
 */
async function parseModrinth(url) {
    try {
        ...
        if (data.project_type === 'plugin')
            categoryHint = 'Plugins';
        else if (data.project_type === 'datapack')
            categoryHint = 'Maps';
        else if (data.project_type === 'modpack')
            categoryHint = 'Mods';
        else if (data.project_type === 'resourcepack')
            categoryHint = 'Other';
        return {
...
async function parseGitHub(url) {
    try {
        ...
        if (data.owner?.avatar_url)
            images.push(data.owner.avatar_url);
        return {
 ...
 */
async function parseCurseForge(url, html) {
    const meta = parseMetaTags(html);
    if (!meta.title)
        return null;
    let categoryHint = 'Mods';
    if (url.includes('/minecraft/mc-mods'))
        categoryHint = 'Mods';
    else if (url.includes('/minecraft/texture-packs'))
        categoryHint = 'Other';
    else if (url.includes('/minecraft/worlds') || url.includes('/minecraft/customization'))
        categoryHint = 'Maps';
 ...
 */
async function parseSpigotMC(url, html) {
    const meta = parseMetaTags(html);
    if (!meta.title)
        return null;
 ...
 */
async function parsePlanetMinecraft(url, html) {
    const meta = parseMetaTags(html);
    if (!meta.title)
        return null;
    let categoryHint = 'Maps';
    if (url.includes('/projects/') || url.includes('/map/'))
        categoryHint = 'Maps';
    else if (url.includes('/texture-packs/'))
        categoryHint = 'Other';
    else if (url.includes('/data-packs/'))
        categoryHint = 'Plugins';
    else if (url.includes('/mods/'))
        categoryHint = 'Mods';
    return {
