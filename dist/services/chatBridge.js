 ...
 */
export async function bridgeDiscordMessageToWeb(message) {
    try {
        if (message.channelId !== PUBLIC_CHAT_CHANNEL_ID)
            return;
        ...
        if (!content)
            return; // skip attachment-only / empty messages for now
        const username = message.member?.displayName || message.author.username || 'Discord user';
        const avatarUrl = message.member?.displayAvatarURL?.({ size: 128 }) ||
