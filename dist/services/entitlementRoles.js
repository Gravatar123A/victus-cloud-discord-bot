...
import { isActiveFreeServer, isActivePaidService, resolveEntitlementRole, } from './entitlementRules.js';
...
async function applyRole(member, entitlement) {
...
function indexUserIdsByEmail(users) {
    ...
    for (const resource of users) {
        ...
        if (!email || id == null)
            continue;
        const ids = result.get(email) || new Set();
        ids.add(String(id));
        result.set(email, ids);
    }
    return result;
}
function ownersWithActiveResources(resources, ownerKey, active) {
    const owners = new Set();
    for (const resource of resources) {
        const item = record(resource);
        const ownerId = ownerKey(item);
        if (ownerId != null && active(resource))
            owners.add(String(ownerId));
    }
    return owners;
}
function activePaidServerServiceIds(servers) {
...
async function loadRoleSyncRecords() {
...
export async function syncEntitlementRoles(client, discordId) {
    ...
    const activeFreeOwners = ownersWithActiveResources(panelServers, (server) => server.user ?? server.owner_id ?? server.user_id, isActiveFreeServer);
    const activePaidOwners = ownersWithActiveResources(billingServices, (service) => service.user_id ?? service.client_id ?? service.user?.id, (service) => {
    ...
    for (const member of guildMembers.values()) {
        ...
        if (hasManagedRole && !linkedDiscordIds.has(member.id) && await applyRole(member, 'none'))
            changed++;
    }
    for (const linked of records.filter((item) => !discordId || item.discordId === discordId)) {
        ...
        if (await applyRole(member, resolveEntitlementRole(hasPaid, hasFree)))
            changed++;
    }
    logger.info(`Victus entitlement role sync complete: ${changed} member(s) updated`);
    return changed;
}
export async function removeEntitlementRoles(member) {
