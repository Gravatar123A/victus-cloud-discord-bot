...
export function isSupabaseCircuitOpen() {
    return Date.now() < circuitOpenUntil;
}
export function tripSupabaseCircuit(reason) {
    consecutiveFailures++;
...
async function resilientFetch(input, init) {
    const isProbe = init?.__isProbe === true;
...
class SupabaseService {
    ...
    async getBotSettings(guildId) {
        ...
        try {
            ...
            let cloudPrefix = data?.prefix || null;
            if (!cloudPrefix) {
                const embedPrefix = await this.getCustomEmbed(guildId, '_bot_prefix').catch(() => null);
                if (embedPrefix?.description)
                    cloudPrefix = embedPrefix.description.trim();
            }
    ...
    async grantInviteCoins(inviterUserId, amount, reference) {
        ...
        try {
            ...
            logger.warn(`grantInviteCoins Paymenter failed for ${inviterUserId}: ${e.message} — falling back to Supabase wallet`);
            try {
                ...
                logger.error(`grantInviteCoins fallback failed for ${inviterUserId}: ${fb.message}`);
            }
    ...
    async grantLevelCoins(userId, level, eventId, amount) {
        ...
        try {
            ...
            logger.warn(`grantLevelCoins Paymenter failed for ${userId}: ${e.message} — falling back to Supabase wallet`);
            try {
                ...
                logger.error(`grantLevelCoins fallback failed for ${userId}: ${fb.message}`);
            }
    ...
    async fallbackIncrementCp(userId, amount, reason) {
        const profile = await this.getUserProfile(userId);
        if (!profile)
            return false;
        const current = Number(profile.total_cp ?? 0);
        ...
        await this.client.from('cp_transactions').insert({ user_id: userId, action_type: `fallback_${reason}`, cp_earned: Math.round(amount), metadata: { reason, fallback: true } }).then(() => { }, () => { });
        return true;
    }
    // ============================================
    ...
    async getInviterCredits(guildId, inviterDiscordId) {
        if (!this.isAvailable())
            return [];
        try {
            const { data, error } = await this.client
    ...
    async getGuildInviteCredits(guildId) {
        if (!this.isAvailable())
            return [];
        try {
            const { data, error } = await this.client
     ...
     */
    async getTicketCategory(id, guildId) {
        if (!id)
            return null;
        ...
        if (guildId) {
            try {
                ...
                if (guildCats && guildCats.length > 0) {
                    const match = guildCats.find((c) => c.id === id ||
