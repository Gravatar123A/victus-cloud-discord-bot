...
export const DEFAULT_UNSCRAMBLE_CONFIG = {
    channelId: null,
    autoLock: true,
...
export class UnscrambleService {
     ...
     */
    async lockChannel(channel, reason = 'Unscramble channel locked') {
        try {
     ...
     */
    async unlockChannel(channel, reason = 'Unscramble channel unlocked') {
        try {
            if (!('permissionOverwrites' in channel))
                return false;
            await channel.permissionOverwrites.edit(channel.guild.roles.everyone, {
    ...
    async setChannel(guild, channel) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription('This channel is the designated **Unscramble The Word** arena!\n\n' +
     ...
     */
    async startGame(guild, channel, hostId, customWord, customCategory) {
        ...
        const startEmbed = new EmbedBuilder()
            ...
            .setDescription(`A new round of **Unscramble The Word** has begun!\n\n` +
            `### Scrambled Word:\n` +
            `# 🧩 \`${spacedScramble}\`\n\n` +
            `🎯 **Goal:** Be the first person to unscramble and type the secret word in chat!\n` +
            `💡 **Clues:** Automatic hints will unlock every ${Math.floor(intervalMs / 1000)}s!\n\n` +
            `🔓 **Channel is now UNLOCKED!** Good luck!`)
            .addFields({ name: '🏷️ Category', value: `\`${targetCategory}\``, inline: true }, { name: '📏 Length', value: `**${targetWord.length} letters**`, inline: true }, {
            name: '🪙 Reward',
    ...
    async endGame(guild, endedByUserId) {
        ...
        if (channel) {
            const endEmbed = new EmbedBuilder()
                ...
                .setDescription(`The round has ended${endedByUserId ? ` by <@${endedByUserId}>` : ''}.\n\n` +
    ...
    async handleMessage(message) {
        ...
        if (cleanContent.length === game.originalWord.length) {
            ...
            for (let i = 0; i < cleanContent.length; i++) {
                if (cleanContent[i] === game.originalWord[i])
                    matches++;
            }
    ...
    async handleWin(message, game) {
        ...
        if (config.rewardCoins > 0) {
            try {
                coinGrantResult = await CoinTransactionLock.grantCoins(message.author.id, config.rewardCoins, 'unscramble_reward', `unscramble:${game.guildId}:${Date.now()}`, `Unscramble Game Reward: Unscrambled word "${game.originalWord}"`);
            }
            catch (err) {
        ...
        const winEmbed = new EmbedBuilder()
            ...
            .setDescription(`🎉 Huge congratulations to <@${message.author.id}> for correctly unscrambling **${game.originalWord}**!\n\n` +
            `The word was accurately identified after **${game.guessCount}** attempt${game.guessCount === 1 ? '' : 's'}!`)
            .addFields({ name: '👤 Winner', value: `<@${message.author.id}> (${message.author.username})`, inline: true }, { name: '🔤 Secret Word', value: `**${game.originalWord}**`, inline: true }, { name: '⏱️ Time Taken', value: `**${durationStr}**`, inline: true }, { name: '📊 Total Guesses', value: `**${game.guessCount}**`, inline: true }, { name: '🏆 Total Wins', value: `**${playerStats.wins}** victor${playerStats.wins === 1 ? 'y' : 'ies'}`, inline: true }, { name: '👥 Total Players', value: `**${game.participants.size}**`, inline: true })
    ...
    async sendHint(guildId, channel, manual = false) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`${hintDesc}\n\n` +
            `🧩 **Scrambled Word:** \`${spacedScramble}\`\n` +
            `🏷️ **Category:** \`${game.category}\``)
            .addFields({ name: '📊 Total Guesses', value: `**${game.guessCount}**`, inline: true }, { name: '👥 Players Active', value: `**${game.participants.size}**`, inline: true })
