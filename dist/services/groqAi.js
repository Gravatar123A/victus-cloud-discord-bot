...
class GroqAiService {
    ...
    async callChatCompletions(messages, withTools, providerOverride) {
        ...
        if (requestedModel)
            return tryModel(requestedModel, isOR ? 30000 : 25000);
        if (!isOR)
            return tryModel(config.ai.model, 25000);
    ...
    async callResponsesApi(messages, withTools) {
    ...
    async complete(messages, allowTools = config.ai.webSearchEnabled, logFailure = true) {
        ...
        if (isAzureResponsesApi(config.ai.baseUrl)) {
            try {
                ...
                if (logFailure)
                    logger.error('Groq AI request failed:', e);
                throw e;
            }
        }
