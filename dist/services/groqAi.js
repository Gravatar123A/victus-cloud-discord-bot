...
function isComplexQuery(t) {
    ...
    if (s.split(/\s+/).length > 28)
        return true;
    if (/(code|build|create|fix|debug|error|stacktrace|exception|analyze|implement|design|develop|script|plugin|config|server\.properties|paper|velocity|bukkit|spigot|purpur|java|how to|tutorial|step\.by\.step|task|project|explain|complex|advanced)/i.test(s))
        return true;
    if (s.includes("```") || (s.includes("{") && s.includes("}")))
        return true;
    return false;
}
function isAzureResponsesApi(baseUrl) {
    return /\/responses(?:\?|$)/i.test(baseUrl);
}
function normalizeEndpoint(baseUrl) {
    const normalized = baseUrl.replace(/\/+$/, '');
    if (isAzureEndpoint(normalized))
        return normalized;
...
function describeUserContext(context) {
    if (!context)
        return 'No Discord user context was provided.';
    const lines = [
...
class GroqAiService {
     ...
     */
    async classifyModeration(content) {
        if (!this.isEnabled())
            return null;
        const messages = [
            {
        ...
        try {
            ...
            logger.debug(`Moderation classification failed: ${error.message}`);
            return null;
        }
    }
    ...
    async suggestForTicket(ticket) {
        ...
        const messages = [
            ...
            {
                role: 'user',
                content: `Create a concise support suggestion for this Victus Cloud ticket. ` +
    ...
    async extractResourceMetadata(url, rawText) {
        if (!this.isEnabled())
            return null;
        try {
            ...
            return {
                ...
                images: Array.isArray(parsed.images) ? parsed.images.map(String).filter((img) => img.startsWith('http')) : undefined,
            };
        }
        catch (error) {
    ...
    async executeStaffTask(prompt, context) {
    ...
    async callChatCompletionsOnce(messages, withTools, model, ms, provider = config.ai) {
        ...
        try {
            ...
            const hasToolHistory = messages.some(m => m.role === 'tool' || (m.role === 'assistant' && Array.isArray(m.tool_calls) && m.tool_calls.length > 0));
    ...
    async callChatCompletions(messages, withTools, providerOverride) {
        const provider = providerOverride || config.ai.primaryProvider || config.ai;
    ...
    async callResponsesApi(messages, withTools, providerOverride) {
        const provider = providerOverride || config.ai.primaryProvider || config.ai;
        ...
        for (let round = 0; round <= MAX_TOOL_ROUNDS; round++) {
            ...
            try {
                ...
                if (!text && Array.isArray(messageItem?.content)) {
                    text = messageItem.content.map((c) => c?.text || '').join('').trim() || undefined;
                }
    ...
    async complete(messages, allowTools = config.ai.webSearchEnabled, logFailure = true) {
        const providers = config.ai.providers?.length
            ? config.ai.providers
            : (config.ai.apiKey ? [{
                    name: 'default',
        ...
        if (logFailure)
            logger.error('All configured AI providers failed. Last error:', lastError);
