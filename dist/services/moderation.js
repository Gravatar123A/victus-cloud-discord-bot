...
function hasExplicitAbuse(content) {
    return EXPLICIT_ABUSE.some((pattern) => pattern.test(content));
}
function detectNonEnglishLocally(content) {
...
async function classifyWithTimeout(content) {
    return Promise.race([
        groqAi.classifyModeration(content),
        new Promise((resolve) => setTimeout(() => resolve(null), 6_000)),
    ]);
}
function policyText(otherLanguageChannelId) {
...
async function sendStaffLog(guild, channelId, text) {
    if (!channelId)
        return;
    ...
    await channel.send({ components: [card], flags: ComponentsV2.IS_COMPONENTS_V2 }).catch(() => undefined);
}
async function getSuspensions(guildId, userId) {
    const record = await supabase.getCustomEmbed(guildId, `_suspensions_${userId}`).catch(() => null);
    if (!record?.description)
        return 0;
    try {
        return Math.max(0, Number(JSON.parse(record.description)?.count || 0));
    }
    catch {
        return 0;
    }
}
async function setSuspensions(guildId, userId, count) {
...
export async function enforceWarningThreshold(guild, target, warningCount, reason) {
    ...
    if (nextSuspensions >= SUSPENSION_LIMIT) {
        await member?.ban({ reason: `Victus Cloud moderation: ${nextSuspensions} suspensions (${reason.slice(0, 300)})` }).catch((error) => {
            logger.warn(`Failed to ban ${target.id} after moderation threshold: ${error.message}`);
        });
        action = 'banned';
    }
    else {
        await member?.timeout(SUSPENSION_MS, `Victus Cloud moderation: suspension ${nextSuspensions}`).catch((error) => {
            logger.warn(`Failed to suspend ${target.id}: ${error.message}`);
        });
    }
...
async function recordAutomaticWarning(message, reason, category, deleteMessage) {
    if (!message.guild)
        return;
    ...
    await sendStaffLog(message.guild, logChannelId, `# 🛡️ AI moderation case\n\n` +
    ...
    const notice = ComponentsV2.moderationWarningContainer(category === 'language' ? 'English is required here' : 'Message removed', publicDescription, message.guild.id, category === 'language' ? settings?.moderation_other_languages_channel_id : null);
    if (message.channel.isTextBased() && 'send' in message.channel) {
        ...
        if (sent)
            setTimeout(() => sent.delete().catch(() => undefined), 5_000).unref?.();
    }
...
export async function inspectModerationMessage(message) {
    ...
    try {
        ...
        if (nonEnglish) {
            await recordAutomaticWarning(message, `Detected language: ${localLanguage?.language || classification?.language || 'non-English'}. ${localLanguage?.reason || 'AI language classifier detected non-English text.'} ${policyText(settings.moderation_other_languages_channel_id)}`, 'language', false);
            return true;
        }
        return false;
    }
