...
let cached: { catalog: PricingCatalog; expiresAt: number } | null = null;
...
export async function fetchPricingCatalog(): Promise<PricingCatalog> {
    ...
    try {
        ...
        const payload = await response.json() as PricingCatalog;
        if (!response.ok || !payload?.version || !Array.isArray(payload.categories)) throw new Error('Invalid pricing catalog');
        cached = { catalog: payload, expiresAt: Date.now() + 5 * 60_000 };
        return payload;
    } catch (error) {
        logger.warn(`Pricing catalog fetch failed; using fallback: ${(error as Error).message}`);
