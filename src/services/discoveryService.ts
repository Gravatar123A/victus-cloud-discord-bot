...
import { pingServerWithFallback, runWithConcurrency, type MinecraftPingResult } from '../utils/minecraftPing.js';

export interface DiscoveredServer {
    serverId: string; // backend_name or unique identifier
    serverName: string;
    description: string;
    category: string;
    categoryLabel: string;
    ip: string;
    connectHostname: string | null;
    ...
    // Discord forum directory tracking
    forumGuildId?: string | null;
    forumThreadId?: string | null;
    forumMessageId?: string | null;
    assignedTagIds?: string[];
    lastSyncedAt?: number;
    lastStatusChangeAt?: number;
    lastLiveUpdateHash?: string;
}
...
export const KNOWN_CATEGORIES: Record<string, { label: string; icon: string; emoji: string }> = {
...
export class DiscoveryService {
    ...
    private fetchPromise: Promise<DiscoveredServer[]> | null = null;

    /**
     * Normalize category string into a recognized category key and label
     */
    public normalizeCategory(raw?: string | null): { key: string; label: string; icon: string; emoji: string } {
     ...
     */
    public normalizeServerRecord(raw: any, existing?: DiscoveredServer): DiscoveredServer {
        ...
        const status: 'online' | 'offline' | 'starting' | 'crashed' = raw.suspended
            ? 'offline'
            : isOnline
            ? 'online'
            : raw.status === 'starting'
        ...
        const planTier: 'free' | 'paid' = raw.type === 'paid' || raw.plan_tier === 'paid' ? 'paid' : 'free';
    ...
    public async ensureLoaded(): Promise<void> {
        ...
        try {
            ...
            const list: DiscoveredServer[] = JSON.parse(raw);
            for (const s of list) {
                if (s.serverId) this.servers.set(s.serverId, s);
            }
        ...
        if (this.servers.size === 0 && supabase.isAvailable()) {
            try {
                ...
                if (embed?.description) {
                    const list: DiscoveredServer[] = JSON.parse(embed.description);
                    for (const s of list) {
                        if (s.serverId) this.servers.set(s.serverId, s);
                    }
     ...
     */
    public async fetchServers(force = false): Promise<DiscoveredServer[]> {
            ...
            try {
                ...
                if (panelToken) {
                    try {
                        ...
                        if (panelResp.ok) {
                            const data: any = await panelResp.json();
                ...
                if (rawList.length > 0) {
                    ...
                    const onlineServers = Array.from(this.servers.values()).filter(
                        (s) => s.discoveryEnabled && !s.suspended && s.status === 'online'
                    );
                        ...
                        await runWithConcurrency(onlineServers, 10, async (s) => {
                            try {
                                ...
                                if (pingRes && pingRes.online) {
                                    ...
                                    if (pingRes.playersMax > 0) s.maxPlayers = pingRes.playersMax;
                                    s.playerSample = pingRes.playerSample;
                                    if (pingRes.versionName) s.version = pingRes.versionName;
     ...
     */
    public async refreshServerLiveStatus(serverId: string): Promise<DiscoveredServer | null> {
        await this.ensureLoaded();
        ...
        try {
            ...
            if (pingRes && pingRes.online) {
                ...
                if (pingRes.playersMax > 0) server.maxPlayers = pingRes.playersMax;
                server.playerSample = pingRes.playerSample;
                if (pingRes.versionName) server.version = pingRes.versionName;
     ...
     */
    public async getFilteredServers(options: ServerFilterOptions = {}): Promise<PaginatedResult<DiscoveredServer>> {
     ...
     */
    public async autocompleteServer(query: string): Promise<Array<{ name: string; value: string }>> {
        const allServers = await this.fetchServers();
     ...
     */
    public async getServer(query: string): Promise<{ exact: DiscoveredServer | null; matches: DiscoveredServer[] }> {
        const allServers = await this.fetchServers();
        ...
        // 2. Exact name match
        const exactName = allServers.find(
            (s) => s.discoveryEnabled && s.serverName.toLowerCase() === clean
        );
        if (exactName) {
     ...
     */
    public async updateForumMetadata(
        serverId: string,
        meta: {
            forumGuildId?: string | null;
            forumThreadId?: string | null;
        ...
        if (meta.forumGuildId !== undefined) s.forumGuildId = meta.forumGuildId;
        if (meta.forumThreadId !== undefined) s.forumThreadId = meta.forumThreadId;
        if (meta.forumMessageId !== undefined) s.forumMessageId = meta.forumMessageId;
        if (meta.assignedTagIds !== undefined) s.assignedTagIds = meta.assignedTagIds;
        if (meta.lastLiveUpdateHash !== undefined) s.lastLiveUpdateHash = meta.lastLiveUpdateHash;
     ...
     */
    public async getCategoryStats(): Promise<Array<{ key: string; label: string; emoji: string; count: number }>> {
        const allServers = await this.fetchServers();
        ...
        const result: Array<{ key: string; label: string; emoji: string; count: number }> = [];
        for (const [key, meta] of Object.entries(KNOWN_CATEGORIES)) {
            if (counts[key]) {
                result.push({
                    key,
     ...
     */
    public async dumpDiscoveredServersJson(): Promise<string> {
