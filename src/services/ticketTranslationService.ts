...
export const TOP_10_LANGUAGES: SupportedLanguage[] = [
...
export interface TicketTranslationState {
    ticketId: string;
    channelId: string;
    customerId: string;
    language: string;       // e.g. 'es'
    languageName: string;   // e.g. 'Spanish'
    languageEmoji: string;  // e.g. '🇪🇸'
    enabled: boolean;
    updatedAt: number;
}
...
class TicketTranslationService {
    ...
    private channelCache = new Map<string, TicketTranslationState>();
     ...
     */
    async getState(channelId: string): Promise<TicketTranslationState | null> {
        ...
        try {
            ...
            if (embed?.description) {
                const parsed: TicketTranslationState = JSON.parse(embed.description);
     ...
     */
    async setState(state: TicketTranslationState): Promise<void> {
    ...
    async setLanguage(
        ...
        languageCode: string,
        enabled = true
    ): Promise<TicketTranslationState> {
        const lang = this.getLanguage(languageCode);
        const state: TicketTranslationState = {
            ticketId,
            channelId,
            customerId,
            language: lang.code,
     ...
     */
    async toggleTranslation(channelId: string, explicitState?: boolean): Promise<TicketTranslationState | null> {
     ...
     */
    private async translateViaGoogle(text: string, targetLang: string, sourceLang = 'auto'): Promise<string> {
     ...
     */
    private async translateViaGroq(text: string, targetLang: string): Promise<string> {
...
${text}`;
    ...
    buildLanguageSelector(
        ticketId: string,
        customerId: string,
        currentState?: TicketTranslationState | null
    ): ContainerBuilder {
        ...
        const container = ComponentsV2.baseContainer(
            isEnabled ? ComponentsV2.Accents.success : ComponentsV2.Accents.primary
        );
        ...
        const description =
            `### 🌐 Ticket Language & Live Translation\n` +
