...
import type { ModrinthCategory } from './modrinthResourceService.js';
 ...
 */
const CURATED_BUILTBYBIT_RESOURCES: Record<ModrinthCategory, UnifiedResourceItem[]> = {
    builds: [
        {
            id: 'bbb-build-1',
...
class BuiltByBitResourceService {
    private readonly apiBase = 'https://api.builtbybit.com/v2';

    /**
     * Map Modrinth categories to BuiltByBit category IDs or search filters
     */
    private getCategoryFilter(category: ModrinthCategory): { categoryId?: number; query?: string } {
        switch (category) {
            case 'builds':
     ...
     */
    private async fetchLiveApiResources(
        category: ModrinthCategory,
        count: number
    ): Promise<UnifiedResourceItem[]> {
        ...
        try {
            ...
            const results: UnifiedResourceItem[] = [];
     ...
     */
    public async fetchFreeResources(
        category: ModrinthCategory,
        count = 100
    ): Promise<UnifiedResourceItem[]> {
