...
export interface BotWorldBoss {
    ...
    current_hp: number;
    pool_coins: number;
    participants_json: Record<string, { damage: number; name: string; hits: number }>;
    channel_ids: string[];
...
export class ViralExpansionStore {
    ...
    async getActiveWorldBoss(): Promise<BotWorldBoss | null> {
        try {
            ...
            if (!error && data) return data as BotWorldBoss;
        } catch {
            // fallback
        }
    ...
    async saveWorldBoss(boss: BotWorldBoss): Promise<void> {
        try {
            await supabase.client
                .from('bot_world_bosses')
                .upsert(boss);
