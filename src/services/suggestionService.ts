...
export interface SuggestionCategoryDef {
    id: SuggestionCategoryKey;
    label: string;
    emoji: string;
    description: string;
    tagLabel: string;
...
export const SUGGESTION_CATEGORIES: Record<SuggestionCategoryKey, SuggestionCategoryDef> = {
    control_panel: {
        id: 'control_panel',
        label: 'Control Panel',
        emoji: '🎛️',
...
export const STATUS_TAGS: Record<string, { name: string; emoji: string }> = {
...
interface LocalSuggestionData {
    nextId: number;
    suggestions: Record<number, Suggestion>;
    votes: Record<number, Record<string, { username: string; vote_type: 'up' | 'down'; created_at: string }>>;
}

const LOCAL_STORE_PATH = join(process.cwd(), 'data', 'suggestions.json');

class SuggestionService {
    private localCache: LocalSuggestionData | null = null;

    private async loadLocalStore(): Promise<LocalSuggestionData> {
     ...
     */
    async getForumChannel(guild: Guild): Promise<ForumChannel | null> {
     ...
     */
    async reconcileForumTags(forumChannel: ForumChannel): Promise<Record<string, string>> {
        ...
        for (const [statusKey, meta] of Object.entries(STATUS_TAGS)) {
            const found = existingTags.find(
                (t) =>
                    t.name.trim().toLowerCase() === meta.name.toLowerCase() ||
        ...
        for (const [catKey, meta] of Object.entries(SUGGESTION_CATEGORIES)) {
            const found = existingTags.find(
                (t) =>
                    t.name.trim().toLowerCase() === meta.tagLabel.toLowerCase() ||
            ...
            try {
                ...
                for (const [statusKey, meta] of Object.entries(STATUS_TAGS)) {
                    if (!tagMap[statusKey]) {
                        ...
                        if (matched) tagMap[statusKey] = matched.id;
                    }
                }
    ...
    async createSuggestionRecord(
        ...
        let dbSuggestion: Suggestion | null = null;
        if (supabase.isAvailable()) {
            try {
                ...
                if (!error && data) {
                    dbSuggestion = data as Suggestion;
    ...
    async getSuggestion(id: number): Promise<Suggestion | null> {
        const store = await this.loadLocalStore();
        if (store.suggestions[id]) return store.suggestions[id];
    ...
    async voteSuggestion(
        ...
        user: User,
        voteType: 'up' | 'down'
    ): Promise<{ up: number; down: number; userVote: 'up' | 'down' | null; removed: boolean }> {
        const store = await this.loadLocalStore();
     ...
     */
    async getVoteCounts(suggestionId: number): Promise<{ up: number; down: number }> {
        const store = await this.loadLocalStore();
     ...
     * Get full vote logs
     */
    async getVotes(suggestionId: number): Promise<SuggestionVote[]> {
        const store = await this.loadLocalStore();
     ...
     */
    async toggleLock(suggestionId: number): Promise<Suggestion | null> {
        const suggestion = await this.getSuggestion(suggestionId);
        if (!suggestion) return null;
     ...
     * Delete suggestion
     */
    async deleteSuggestion(suggestionId: number): Promise<boolean> {
        const store = await this.loadLocalStore();
