...
export interface GtnConfig {
    ...
    maxNumber: number;             // Maximum number for random secret in auto games (default 10000)
...
export class GtnService {
    ...
    async startGame(
        ...
        const startEmbed = new EmbedBuilder()
            ...
            .addFields(
                { name: '👑 Host', value: hostId === guild.client.user?.id ? '🤖 Victus Cloud (Automated)' : `<@${hostId}>`, inline: true },
                { name: '📊 Range', value: `\`1\` ⟷ \`${maxRange}\``, inline: true },
                {
                    name: '🪙 Reward',
    ...
    private async handleWin(message: Message, game: ActiveGtnGame): Promise<void> {
        ...
        if (config.rewardCoins > 0) {
            try {
                coinGrantResult = await CoinTransactionLock.grantCoins(
                    ...
                    config.rewardCoins,
                    'gtn_reward',
                    `gtn:${game.guildId}:${Date.now()}`,
                    `GTN Game Reward: Guessed secret number ${game.secretNumber}`
                );
            } catch (err) {
        ...
        const winEmbed = new EmbedBuilder()
            ...
            .setDescription(
                `🎉 Huge congratulations to <@${message.author.id}> for correctly guessing the secret number **${game.secretNumber}**!\n\n` +
