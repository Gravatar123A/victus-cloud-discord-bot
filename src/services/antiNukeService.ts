...
async function isWhitelisted(guild: Guild, userId: string): Promise<boolean> {
    ...
    if (user && await isGrav(user, guild.client, guild)) return true;

    // Check Extra Owners (User ID or Role Holder)
    const member = await guild.members.fetch(userId).catch(() => null);
    if (user && await extraOwnerSettings.isExtraOwner(guild, user, member)) return true;
 ...
 */
async function sendAlert(guild: Guild, title: string, description: string, isExemption = false) {
    try {
...
export class AntiNukeService {
    async handleMemberRemove(member: GuildMember) {
        try {
            ...
            if (whitelisted) {
                await sendAlert(member.guild, 'Exemption Verified',
                    `### 🛡️ Authorized Action Permitted\n\n` +
                    `> **Security Module:** 👢 \`ANTI-KICK SHIELD\`\n` +
                    `> **Authorized Actor:** <@${executor.id}> (\`${executor.username}\`)\n` +
                    `> **Exemption Tier:** 👑 Grav / Extra Owner / Whitelist\n` +
                    `> **Action:** Kicked member <@${member.id}> without penalty.`,
                    true
                );
                return;
            }
            ...
            await sendAlert(member.guild, 'Kick Protection Triggered',
                `### 🚨 Unauthorized Member Kick Intercepted\n\n` +
                `> **Security Module:** 👢 \`ANTI-KICK SHIELD\`\n` +
    ...
    async handleBanAdd(ban: GuildBan) {
        try {
            ...
            if (whitelisted) {
                await sendAlert(ban.guild, 'Exemption Verified',
                    `### 🛡️ Authorized Action Permitted\n\n` +
                    `> **Security Module:** 🔨 \`ANTI-BAN SHIELD\`\n` +
                    `> **Authorized Actor:** <@${executor.id}> (\`${executor.username}\`)\n` +
                    `> **Exemption Tier:** 👑 Grav / Extra Owner / Whitelist\n` +
                    `> **Action:** Banned user <@${ban.user.id}> without penalty.`,
                    true
                );
                return;
            }
            ...
            await sendAlert(ban.guild, 'Ban Protection Triggered',
                `### 🚨 Unauthorized Member Ban Intercepted\n\n` +
                `> **Security Module:** 🔨 \`ANTI-BAN SHIELD\`\n` +
                `> **Perpetrator:** <@${executor.id}> (\`${executor.username}\` · \`${executor.id}\`)\n` +
        ...
        try {
            ...
            if (whitelisted) {
                await sendAlert(ban.guild, 'Exemption Verified',
                    ...
                    `> **Authorized Actor:** <@${executor.id}> (\`${executor.username}\`)\n` +
                    `> **Exemption Tier:** 👑 Grav / Extra Owner / Whitelist\n` +
                    `> **Action:** Unbanned user <@${ban.user.id}> without penalty.`,
                    true
                );
                return;
            }
            ...
            await sendAlert(ban.guild, 'Ban-Remove Protection Triggered',
                `### 🚨 Unauthorized Member Unban Intercepted\n\n` +
                `> **Security Module:** 🔓 \`ANTI-BAN-REMOVE SHIELD\`\n` +
                `> **Perpetrator:** <@${executor.id}> (\`${executor.username}\` · \`${executor.id}\`)\n` +
    ...
    async handleChannelCreate(channel: any) {
        try {
            ...
            if (whitelisted) {
                await sendAlert(channel.guild, 'Exemption Verified',
                    `### 🛡️ Authorized Action Permitted\n\n` +
                    `> **Security Module:** ➕ \`ANTI-CHANNEL-CREATE SHIELD\`\n` +
                    `> **Authorized Actor:** <@${executor.id}> (\`${executor.username}\`)\n` +
                    `> **Exemption Tier:** 👑 Grav / Extra Owner / Whitelist\n` +
                    `> **Action:** Created channel \`${channel.name}\` without penalty.`,
                    true
                );
                return;
            }
            ...
            await sendAlert(channel.guild, 'Channel-Create Protection Triggered',
                `### 🚨 Unauthorized Channel Creation Intercepted\n\n` +
                `> **Security Module:** ➕ \`ANTI-CHANNEL-CREATE SHIELD\`\n` +
                `> **Perpetrator:** <@${executor.id}> (\`${executor.username}\` · \`${executor.id}\`)\n` +
        ...
        try {
            ...
            if (whitelisted) {
                await sendAlert(channel.guild, 'Exemption Verified',
                    ...
                    `> **Authorized Actor:** <@${executor.id}> (\`${executor.username}\`)\n` +
                    `> **Exemption Tier:** 👑 Grav / Extra Owner / Whitelist\n` +
                    `> **Action:** Deleted channel \`${channel.name}\` without penalty.`,
                    true
                );
                return;
            }
            ...
            await sendAlert(channel.guild, 'Channel-Delete Protection Triggered',
                `### 🚨 Unauthorized Channel Deletion Intercepted\n\n` +
                `> **Security Module:** ❌ \`ANTI-CHANNEL-DELETE SHIELD\`\n` +
                `> **Perpetrator:** <@${executor.id}> (\`${executor.username}\` · \`${executor.id}\`)\n` +
    ...
    async handleRoleCreate(role: any) {
        try {
            ...
            if (whitelisted) {
                await sendAlert(role.guild, 'Exemption Verified',
                    `### 🛡️ Authorized Action Permitted\n\n` +
                    `> **Security Module:** 🎭 \`ANTI-ROLE-CREATE SHIELD\`\n` +
                    `> **Authorized Actor:** <@${executor.id}> (\`${executor.username}\`)\n` +
                    `> **Exemption Tier:** 👑 Grav / Extra Owner / Whitelist\n` +
                    `> **Action:** Created role \`${role.name}\` without penalty.`,
                    true
                );
                return;
            }
            ...
            await sendAlert(role.guild, 'Role-Create Protection Triggered',
                `### 🚨 Unauthorized Role Creation Intercepted\n\n` +
                `> **Security Module:** 🎭 \`ANTI-ROLE-CREATE SHIELD\`\n` +
                `> **Perpetrator:** <@${executor.id}> (\`${executor.username}\` · \`${executor.id}\`)\n` +
        ...
        try {
            ...
            if (whitelisted) {
                await sendAlert(role.guild, 'Exemption Verified',
                    ...
                    `> **Authorized Actor:** <@${executor.id}> (\`${executor.username}\`)\n` +
                    `> **Exemption Tier:** 👑 Grav / Extra Owner / Whitelist\n` +
                    `> **Action:** Deleted role \`${role.name}\` without penalty.`,
                    true
                );
                return;
            }
            ...
            await sendAlert(role.guild, 'Role-Delete Protection Triggered',
                `### 🚨 Unauthorized Role Deletion Intercepted\n\n` +
                `> **Security Module:** 🗑️ \`ANTI-ROLE-DELETE SHIELD\`\n` +
                `> **Perpetrator:** <@${executor.id}> (\`${executor.username}\` · \`${executor.id}\`)\n` +
    ...
    async handleRoleUpdate(oldRole: any, newRole: any) {
        try {
            ...
            if (whitelisted) {
                await sendAlert(newRole.guild, 'Exemption Verified',
                    `### 🛡️ Authorized Action Permitted\n\n` +
                    `> **Security Module:** 📝 \`ANTI-ROLE-UPDATE SHIELD\`\n` +
                    `> **Authorized Actor:** <@${executor.id}> (\`${executor.username}\`)\n` +
                    `> **Exemption Tier:** 👑 Grav / Extra Owner / Whitelist\n` +
                    `> **Action:** Modified role \`${newRole.name}\` without penalty.`,
                    true
                );
                return;
            }
            ...
            await sendAlert(newRole.guild, 'Role-Update Protection Triggered',
                `### 🚨 Unauthorized Dangerous Role Update Intercepted\n\n` +
                `> **Security Module:** 📝 \`ANTI-ROLE-UPDATE SHIELD\`\n` +
                `> **Perpetrator:** <@${executor.id}> (\`${executor.username}\` · \`${executor.id}\`)\n` +
    ...
    async handleEmojiDelete(emoji: any) {
        try {
            ...
            if (whitelisted) {
                await sendAlert(emoji.guild, 'Exemption Verified',
                    `### 🛡️ Authorized Action Permitted\n\n` +
                    `> **Security Module:** 😀 \`ANTI-EMOJI-DELETE SHIELD\`\n` +
                    `> **Authorized Actor:** <@${executor.id}> (\`${executor.username}\`)\n` +
                    `> **Exemption Tier:** 👑 Grav / Extra Owner / Whitelist\n` +
                    `> **Action:** Deleted emoji \`${emoji.name}\` without penalty.`,
                    true
                );
                return;
            }
            ...
            await sendAlert(emoji.guild, 'Emoji-Delete Protection Triggered',
                `### 🚨 Unauthorized Emoji Deletion Intercepted\n\n` +
                `> **Security Module:** 😀 \`ANTI-EMOJI-DELETE SHIELD\`\n` +
                `> **Perpetrator:** <@${executor.id}> (\`${executor.username}\` · \`${executor.id}\`)\n` +
    ...
    async handleStickerDelete(sticker: any) {
        try {
            ...
            if (whitelisted) {
                await sendAlert(sticker.guild, 'Exemption Verified',
                    `### 🛡️ Authorized Action Permitted\n\n` +
                    `> **Security Module:** 🏷️ \`ANTI-STICKER-DELETE SHIELD\`\n` +
                    `> **Authorized Actor:** <@${executor.id}> (\`${executor.username}\`)\n` +
                    `> **Exemption Tier:** 👑 Grav / Extra Owner / Whitelist\n` +
                    `> **Action:** Deleted sticker \`${sticker.name}\` without penalty.`,
                    true
                );
                return;
            }
            ...
            await sendAlert(sticker.guild, 'Sticker-Delete Protection Triggered',
                `### 🚨 Unauthorized Sticker Deletion Intercepted\n\n` +
                `> **Security Module:** 🏷️ \`ANTI-STICKER-DELETE SHIELD\`\n` +
                `> **Perpetrator:** <@${executor.id}> (\`${executor.username}\` · \`${executor.id}\`)\n` +
    ...
    async handleGuildUpdate(oldGuild: any, newGuild: any) {
        try {
            ...
            if (whitelisted) {
                await sendAlert(newGuild, 'Exemption Verified',
                    `### 🛡️ Authorized Action Permitted\n\n` +
                    `> **Security Module:** 🌐 \`ANTI-GUILD-UPDATE SHIELD\`\n` +
                    `> **Authorized Actor:** <@${executor.id}> (\`${executor.username}\`)\n` +
                    `> **Exemption Tier:** 👑 Grav / Extra Owner / Whitelist\n` +
                    `> **Action:** Updated server settings without penalty.`,
                    true
                );
                return;
            }
            ...
            await sendAlert(newGuild, 'Guild-Update Protection Triggered',
                `### 🚨 Unauthorized Server Configuration Modification Intercepted\n\n` +
                `> **Security Module:** 🌐 \`ANTI-GUILD-UPDATE SHIELD\`\n` +
                `> **Perpetrator:** <@${executor.id}> (\`${executor.username}\` · \`${executor.id}\`)\n` +
                `> **Mitigation Action:** ${punishment}\n` +
