...
import { DiscoveredServer, PaginatedResult, KNOWN_CATEGORIES } from '../services/discoveryService.js';
...
export class DiscoveryEmbeds {
     ...
     */
    public static getStatusBadge(status: 'online' | 'offline' | 'starting' | 'crashed'): { dot: string; label: string } {
        switch (status) {
            case 'online':
     ...
     */
    public static buildBrowser(
        data: PaginatedResult<DiscoveredServer>,
        state: BrowserSessionState
    ): { container: ContainerBuilder; actionRows: ActionRowBuilder<any>[] } {
        ...
        const actionRows: ActionRowBuilder<any>[] = [];
        ...
        actionRows.push(new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(categorySelect));
        ...
        actionRows.push(new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(sortSelect));
        ...
        actionRows.push(
            new ActionRowBuilder<ButtonBuilder>().addComponents(prevBtn, pageIndicatorBtn, nextBtn, searchBtn, clearSearchBtn)
        );
     ...
     */
    public static buildServerStatusCard(server: DiscoveredServer): {
