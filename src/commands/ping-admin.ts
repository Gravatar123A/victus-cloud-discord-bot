...
function createDiagnosticsEmbed(
    tierName: string,
    wsDisplay: string,
    apiLatency: number,
    ...
    return new EmbedBuilder()
        ...
        .setDescription(
            `-# Operational state verified • Access granted to **${tierName}**\n\n` +
            `### 🏓 Latency & Network\n` +
...
export const pingAdminCommand: Command = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        try {
            ...
            const diagnosticsEmbed = createDiagnosticsEmbed(
                auth.tierName || 'Staff Member',
                wsDisplay,
                apiLatency,
                formattedUptime,
                restartTimestamp,
    ...
    async handleButton(interaction: ButtonInteraction) {
        if (interaction.customId !== REFRESH_CUSTOM_ID) return;
