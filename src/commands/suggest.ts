...
export const suggestCommand: Command = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((o) =>
            o.setName('title').setDescription('Optional: Direct title for fast submission').setRequired(false).setMaxLength(100)
        )
        .addStringOption((o) =>
            o.setName('content').setDescription('Optional: Direct proposal details').setRequired(false).setMaxLength(1500)
        ),
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (!forumChannel) {
            const errCard = ComponentsV2.errorContainer(
                'Suggestions Not Configured',
                'No **Forum Channel** has been configured for suggestions yet.\n\nAn administrator must set one up by running `/suggestion config`.'
            );
        ...
        const initialCategories: SuggestionCategoryKey[] = [];
        ...
        if (directTitle && directContent) {
            ...
            hintCard.addTextDisplayComponents(
                ComponentsV2.text(
    ...
    async handleSelectMenu(interaction: StringSelectMenuInteraction) {
        if (interaction.customId !== 'suggest:category_select') return;

        const selectedKeys = interaction.values as SuggestionCategoryKey[];
    ...
    async handleButton(interaction: ButtonInteraction) {
        ...
        if (customId.startsWith('suggest:open_modal:')) {
            ...
            modal.addComponents(
                ...
                new ActionRowBuilder<ModalActionRowComponentBuilder>().addComponents(impactInput)
            );
        ...
        if (customId.startsWith('suggest:upvote:') || customId.startsWith('suggest:downvote:')) {
            ...
            const voteType = parts[1] as 'upvote' | 'downvote';
            ...
            const { container, rows } = SuggestionEmbeds.buildSuggestionCard(
                suggestion,
                result.up,
                result.down,
                result.userVote
        ...
        if (customId.startsWith('suggest_mod:')) {
            ...
            if (action === 'implement') {
                ...
                modal.addComponents(new ActionRowBuilder<ModalActionRowComponentBuilder>().addComponents(changelogInput));
                await interaction.showModal(modal);
                return;
            }
            ...
            if (action === 'delete') {
                ...
                if (interaction.channel?.isThread()) {
                    await (interaction.channel as ThreadChannel).delete('Suggestion deleted by staff').catch(() => {});
                } else {
    ...
    async handleModal(interaction: ModalSubmitInteraction) {
        ...
        if (customId.startsWith('suggest_modal:submit:')) {
            ...
            const selectedCategoryKeys = (rawCats && rawCats !== 'none'
                ? rawCats.split(',')
                : ['general']) as SuggestionCategoryKey[];
            ...
            if (!forumChannel) {
                const errCard = ComponentsV2.errorContainer(
                    'Suggestions Not Configured',
                    'The Forum Channel for suggestions has not been configured or is missing. Please contact staff.'
                );
            ...
            try {
                ...
                const { container: initialCard, rows: initialRows } = SuggestionEmbeds.buildSuggestionCard(
                ...
                const createdSuggestion = await suggestionService.createSuggestionRecord(
                ...
                if (starterMessage) {
                    const { container: finalCard, rows: finalRows } = SuggestionEmbeds.buildSuggestionCard(
                        createdSuggestion,
                        0,
                        0
                    );
                ...
                const successCard = ComponentsV2.successContainer(
                    'Suggestion Published!',
                    `Your suggestion **"${title}"** has been published to <#${forumChannel.id}>!\n\n👉 **[Jump to your suggestion post](${thread.url})** to follow community feedback.`
                );
                ...
                const errCard = ComponentsV2.errorContainer(
                    'Submission Failed',
                    `Could not publish your suggestion to the forum channel: \`${err?.message || 'Unknown error'}\``
                );
                await interaction.editReply({ components: [errCard], flags: V2 });
            }
            return;
 ...
 */
async function syncForumThreadReview(
    interaction: ModalSubmitInteraction,
    suggestion: any,
    decision: 'approved' | 'denied' | 'implemented',
    reason?: string
) {
        try {
            const forumChannel = await suggestionService.getForumChannel(interaction.guild!);
            ...
            if (thread && thread.isThread()) {
                ...
                const statusPrefix =
                    decision === 'approved' ? '[Accepted]' : decision === 'denied' ? '[Rejected]' : '[Implemented]';
                ...
                await thread.setAppliedTags(nextTags.slice(0, 5)).catch((err) =>
                    logger.warn('Failed to update forum thread tags:', err)
                );
                ...
                const starterMessage = await (thread as ThreadChannel).fetchStarterMessage().catch(() => null);
                if (starterMessage) {
                    await starterMessage.edit({
                        components: [container, ...rows],
                        flags: V2,
                ...
                if (decision === 'approved') {
                    await (thread as ThreadChannel).send({
                        content:
                            `🎉 <@${suggestion.user_id}> Your suggestion was **ACCEPTED** by <@${interaction.user.id}>!\n` +
                            (reason ? `> **Staff Note:** ${reason}` : ''),
                    }).catch(() => {});
                } else if (decision === 'denied') {
                    await (thread as ThreadChannel).send({
                        content:
                            `❌ <@${suggestion.user_id}> Your suggestion was **DECLINED** by <@${interaction.user.id}>.\n` +
                            `> **Reason:** ${reason || 'Declined by staff team.'}`,
                    }).catch(() => {});
                } else if (decision === 'implemented') {
                    await (thread as ThreadChannel).send({
                        content:
                            `🚀 <@${suggestion.user_id}> Your suggestion has been **IMPLEMENTED** by <@${interaction.user.id}>!\n` +
...
export const suggestionCommand: Command = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((sub) =>
            sub.setName('status').setDescription('View current suggestion forum channel and tag configuration')
        ),
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (sub === 'config') {
            ...
            const statusCount = Object.keys(tagMap).filter((k) =>
                ['pending', 'accepted', 'rejected', 'implemented'].includes(k)
            ).length;
            const categoryCount = Object.keys(tagMap).length - statusCount;

            const setupCard = ComponentsV2.successContainer(
                'Suggestions Forum Configured',
                `Target forum channel set to <#${channel.id}>.\n\n` +
        ...
        if (sub === 'status') {
            ...
            if (!forumChannel) {
                const unconfigured = ComponentsV2.warningContainer(
                    'Suggestions Not Configured',
                    'No Forum channel is currently assigned for suggestions. Run `/suggestion config` to set one.'
                );
            ...
            const statusCard = ComponentsV2.infoContainer(
                'Suggestions Forum Status',
                `**Configured Channel:** <#${forumChannel.id}>\n` +
