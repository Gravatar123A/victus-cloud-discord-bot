...
import { isGrav, canManageSecurity } from '../utils/securityAuth.js';
...
export const extraOwnerCommand: Command = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((sub) =>
            sub
                ...
                .addUserOption((opt) =>
                    opt.setName('user').setDescription('User to designate as Extra Owner').setRequired(false)
                )
                .addRoleOption((opt) =>
                    opt.setName('role').setDescription('Role to designate as Extra Owner (all members gain access)').setRequired(false)
                )
        )
        .addSubcommand((sub) =>
            sub
                ...
                .addUserOption((opt) =>
                    opt.setName('user').setDescription('User to remove from Extra Owners').setRequired(false)
                )
                .addRoleOption((opt) =>
                    opt.setName('role').setDescription('Role to remove from Extra Owners').setRequired(false)
                )
        )
        .addSubcommand((sub) =>
            sub
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (sub === 'list') {
            ...
            c.addTextDisplayComponents(
                ComponentsV2.text(
                    `# 👑 Extra Owners: ${interaction.guild.name}\n\n` +
