...
import { viralExpansionStore, CapturedMob } from '../services/viralExpansionStore.js';
...
export const tameCommand: Command = {
...
export const zooCommand: Command = {
    data: new SlashCommandBuilder()
        .setName('zoo')
    ...
    async execute(interaction) {
        ...
        collector.on('collect', async (btn) => {
            ...
            const chalDeduct = await CoinTransactionLock.executeWagerTransaction(
                ...
                wager,
                'battle_wager',
                `duel:${challenger.id}:${Date.now()}`,
                `Duel wager vs ${opponent.username}`,
            ...
            const oppDeduct = await CoinTransactionLock.executeWagerTransaction(
                ...
                wager,
                'battle_wager',
                `duel:${opponent.id}:${Date.now()}`,
                `Duel wager vs ${challenger.username}`,
                async () => ({ won: false, payoutAmount: 0, payload: null })
            );
            ...
            await CoinTransactionLock.grantCoins(
                ...
                totalPot,
                'battle_win',
                `duel_pot:${winner.id}:${Date.now()}`,
                `Won duel against ${challengerWon ? opponent.username : challenger.username} (+${totalPot} COINS)`
            );
