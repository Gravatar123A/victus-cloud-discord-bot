...
const CATEGORIES = ['Maps', 'Builds', 'Lobbies', 'Plugins', 'Mods', 'Bots', 'Codes', 'Other'] as const;
 ...
 */
function parseResourceInput(rawContent: string): { items: RawResourceItem[]; error?: string } {
    ...
    if (trimmed.startsWith('[') || trimmed.startsWith('{')) {
        try {
            ...
            for (const entry of array) {
                ...
                const tags = Array.isArray(entry.tags) ? entry.tags.map(String).map((t: string) => t.trim()).filter(Boolean) : [];
                const images = Array.isArray(entry.images) ? entry.images.map(String).map((i: string) => i.trim()).filter(Boolean) : (typeof entry.image === 'string' ? [entry.image.trim()] : []);
...
export const massShareResourceCommand: Command = {
    ...
    async execute(interaction) {
            ...
            await interaction.reply({
                components: [
                    ComponentsV2.errorContainer(
                        'Access Denied',
                        'You do not have permission to use `/mass-share-resource`.\n\n' +
        ...
        const defaultCategory = (interaction.options.getString('default_category') as CategoryType | null) || 'Other';
        ...
        if (!fileAttachment && !jsonString) {
            const guideContainer = ComponentsV2.infoContainer(
                'Mass Resource Ingestion Guide',
                'Upload multiple resources directly to the forum channel at once!\n\n' +
                '### 📥 Usage Options\n' +
        ...
        const forumTagMap = new Map<string, { id: string; name: string }>();
            ...
            try {
                ...
                await interaction.editReply({
                    components: [
                        ComponentsV2.errorContainer(
                            'File Download Error',
                            `Could not download attached file: ${fetchErr?.message || 'Network error'}`
                        ),
                    ],
                    flags: ComponentsV2.IS_COMPONENTS_V2,
                });
            ...
            await interaction.editReply({
                components: [
                    ComponentsV2.errorContainer(
                        ...
                        parseResult.error || 'No valid resource definitions could be found.'
                    ),
                ],
                flags: ComponentsV2.IS_COMPONENTS_V2,
            });
        ...
        await interaction.editReply({
            components: [
                ComponentsV2.cleanContainer(
                    ...
                    'Starting Mass Resource Ingestion...',
                    `### \`[${initialBar}]\` **0%** (0/${total})\n\n` +
                    ...
                    `-# Initializing forum ingestion pipeline...`,
                    'MASS INGESTION'
                ),
            ],
            flags: ComponentsV2.IS_COMPONENTS_V2,
        ...
        for (let i = 0; i < total; i++) {
            ...
            let category = (rawItem.category && CATEGORIES.includes(rawItem.category as CategoryType))
                ? (rawItem.category as CategoryType)
                : defaultCategory;
            ...
            if (rawItem.isUrlOnly && sourceUrl) {
                try {
                    ...
                    if (scraped.category_hint && CATEGORIES.includes(scraped.category_hint as CategoryType)) {
                        category = scraped.category_hint as CategoryType;
                    }
                    if (scraped.author) author = scraped.author;
                    if (scraped.images && scraped.images.length > 0) images = scraped.images;
            ...
            const candidateTagKeywords: string[] = [
            ...
            const postEmbed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: '🔗 Source Page', value: sourceUrl ? `[View Original Link](${sourceUrl})` : 'N/A', inline: false }
                );
            ...
            await interaction.editReply({
                components: [
                    ComponentsV2.cleanContainer(
                        ComponentsV2.Accents.primary,
                        'Ingesting Resources...',
                        `### \`[${progressBar}]\` **${percent}%** (${currentItemNumber}/${total})\n\n` +
                        `› **Recent Item:** \`${title.slice(0, 50)}\`\n` +
                        `› **Target Forum:** <#${forumChannel.id}>\n` +
                        `› **Tags Applied:** ${matchedTagNames.length > 0 ? matchedTagNames.map((t) => `\`${t}\``).join(', ') : 'None'}\n\n` +
                        `-# Creating forum listings with strict existing-tag validation...`,
                        'MASS INGESTION IN PROGRESS'
                    ),
                ],
                flags: ComponentsV2.IS_COMPONENTS_V2,
            }).catch(() => {});
        ...
        const summaryContainer = ComponentsV2.cleanContainer(
            failed.length === 0 ? ComponentsV2.Accents.success : ComponentsV2.Accents.warning,
