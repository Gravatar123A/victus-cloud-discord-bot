        ...
        try {
            if (subcommand === 'view') {
                ...
                const container = ComponentsV2.infoContainer(
                    'Level Announcements Channel',
                    `Level-up and rank-up notifications are currently broadcast in <#${currentChannelId}>.\n\n` +
            ...
            if (subcommand === 'set') {
                ...
                const container = ComponentsV2.successContainer(
                    'Level Announcements Channel Updated',
                    `Level-up and rank-up announcements will now be sent to <#${channel.id}>.`
                );
                await interaction.editReply({
                    components: [container],
                    flags: ComponentsV2.IS_COMPONENTS_V2,
