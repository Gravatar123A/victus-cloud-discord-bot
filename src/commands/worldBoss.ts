...
function renderHealthBar(current: number, max: number, size = 15): string {
...
export const attackCommand: Command = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((opt) =>
            opt
                ...
                .setRequired(false)
                .addChoices(
                    { name: '⚔️ Melee Slash (Reliable)', value: 'melee' },
                    { name: '🏹 Bow Shot (Balanced)', value: 'bow' },
                    { name: '✨ Magic Blast (High Variance)', value: 'magic' }
                )
        ),
    ...
    async execute(interaction) {
        const type = (interaction.options.getString('type') || 'melee') as 'melee' | 'bow' | 'magic';
        ...
        const container = ComponentsV2.baseContainer(
            result.defeated ? ComponentsV2.Accents.success : ComponentsV2.Accents.primary
        );
...
export const castCommand: Command = {
    ...
    async execute(interaction) {
        (interaction.options as any).getString = (name: string) => name === 'type' ? 'magic' : null;
