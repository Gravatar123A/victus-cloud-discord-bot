...
export const coinflipCommand: Command = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((opt) =>
            opt
                ...
                .setRequired(false)
                .addChoices(
                    { name: '🪙 Heads (h)', value: 'heads' },
                    { name: '🪙 Tails (t)', value: 'tails' }
                )
        ),
    ...
    async execute(interaction) {
        let rawAmountStr: string | number | null = interaction.options.getString?.('amount') ?? interaction.options.getInteger?.('amount') ?? null;
        let choice: string | null = interaction.options.getString?.('choice') ?? null;
        ...
        const tx = await CoinTransactionLock.executeWagerTransaction(
            userId,
            amount,
            'gambling_coinflip',
            `cf:${userId}:${Date.now()}`,
            `Coinflip wager of ${amount} COINS on ${choice}`,
            async () => {
        ...
        const container = ComponentsV2.baseContainer(
            isWin ? ComponentsV2.Accents.success : ComponentsV2.Accents.danger
        );
...
export const slotsCommand: Command = {
    ...
    async execute(interaction) {
        ...
        const tx = await CoinTransactionLock.executeWagerTransaction(
            ...
            amount,
            'gambling_slots',
            `slots:${userId}:${Date.now()}`,
            `Slots spin wager of ${amount} COINS`,
            async (user) => {
                ...
                // 2 of a kind (consolation)
                    ...
                    (reel1.icon === reel2.icon || reel2.icon === reel3.icon || reel1.icon === reel3.icon) &&
...
export const heistCommand: Command = {
    data: new SlashCommandBuilder()
        .setName('heist')
    ...
    async execute(interaction) {
        ...
        const buildLobbyText = (secondsLeft: number) => {
        ...
        collector.on('end', async () => {
            ...
            const endContainer = ComponentsV2.baseContainer(
                heistSuccess ? ComponentsV2.Accents.success : ComponentsV2.Accents.danger
            );
            ...
            const heistRes = await CoinTransactionLock.executeTransferOrHeist(
                targetUser.id,
                Array.from(crew),
                amount,
                'gambling_heist',
                `heist:${targetUser.id}:${Date.now()}`,
                `Vault heist by squad of ${squadSize}`
            );
...
async function executeRpsDuel(
    interaction: any,
    challenger: User,
    opponent: User,
    rawAmountStr: string | number | null
) {
    ...
    const challengeRow = new ActionRowBuilder<ButtonBuilder>().addComponents(acceptBtn, declineBtn);
    ...
    inviteCollector.on('collect', async (btn: any) => {
        ...
        const chalDeduct = await CoinTransactionLock.executeWagerTransaction(
            challenger.id,
            wager,
            'rps_duel_wager',
            `rps_duel:${challenger.id}:${Date.now()}`,
            `RPS duel wager vs ${opponent.username}`,
            async () => ({ won: false, payoutAmount: 0, payload: null })
        );
        ...
        const oppDeduct = await CoinTransactionLock.executeWagerTransaction(
            opponent.id,
            wager,
            'rps_duel_wager',
            `rps_duel:${opponent.id}:${Date.now()}`,
            `RPS duel wager vs ${challenger.username}`,
            async () => ({ won: false, payoutAmount: 0, payload: null })
        );
        ...
        const choices: Record<string, 'rock' | 'paper' | 'scissors'> = {};
        ...
        moveCollector.on('collect', async (moveBtn: any) => {
            ...
            let pickedMove: 'rock' | 'paper' | 'scissors';
            if (moveBtn.customId.includes('rock')) pickedMove = 'rock';
            else if (moveBtn.customId.includes('paper')) pickedMove = 'paper';
            else pickedMove = 'scissors';
        ...
        moveCollector.on('end', async (_: any, reason: string) => {
            ...
            if (chalMove && oppMove) {
                ...
                const chalWins =
                    (chalMove === 'rock' && oppMove === 'scissors') ||
                ...
                await CoinTransactionLock.grantCoins(
                    ...
                    totalPot,
                    'rps_duel_win',
                    `rps_pot:${winner.id}:${Date.now()}`,
                    `Won RPS duel against ${loser.username} (+${totalPot} COINS)`
                );

                const winVerb =
                    winningMove === 'rock' ? 'smashes' : winningMove === 'paper' ? 'covers' : 'cuts';
            ...
            if (chalMove && !oppMove) {
                ...
                fContainer.addTextDisplayComponents(ComponentsV2.text(
                    `# 🏆 Duel Won by Forfeit!\n\n` +
            ...
            expContainer.addTextDisplayComponents(ComponentsV2.text(
                `# ⏳ Duel Expired\n\n` +
    ...
    inviteCollector.on('end', async (_: any, reason: string) => {
        if (reason === 'time') {
            ...
            timeoutC.addTextDisplayComponents(ComponentsV2.text(
                `# ⏳ Challenge Expired\n\n` +
...
export const rpsCommand: Command = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((opt) =>
            opt
                ...
                .setRequired(false)
                .addChoices(
                    { name: '🪨 Rock (r)', value: 'rock' },
                    { name: '📰 Paper (p)', value: 'paper' },
                    { name: '✂️ Scissors (s)', value: 'scissors' }
                )
        )
        .addUserOption((opt) =>
            opt
    ...
    async execute(interaction) {
        let rawAmountStr: string | number | null = interaction.options.getString?.('amount') ?? interaction.options.getInteger?.('amount') ?? null;
        let choice: string | null = interaction.options.getString?.('choice') ?? null;
        let opponent: User | null = interaction.options.getUser?.('opponent') ?? null;
        const userId = interaction.user.id;
        ...
        if (rawMessage?.content) {
            if (rawMessage.mentions?.users?.size > 0) {
                const mentioned = rawMessage.mentions.users.find((u: User) => u.id !== interaction.user.id);
                if (mentioned) opponent = mentioned;
            }
        ...
        const tx = await CoinTransactionLock.executeWagerTransaction(
            ...
            'gambling_rps',
            `rps:${userId}:${Date.now()}`,
            `RPS wager of ${amount} COINS on ${choice}`,
            async () => {
...
export const rpsDuelCommand: Command = {
    data: new SlashCommandBuilder()
        .setName('rps-duel')
...
function calculateHand(hand: Card[]): { total: number; isBust: boolean; isBlackjack: boolean } {
    let total = 0;
    let aces = 0;
    for (const c of hand) {
        total += c.value;
...
function formatCards(hand: Card[], hideSecond = false): string {
    ...
    let tag = `Value: **${total}**`;
    if (isBlackjack) tag = `Value: **21 (Blackjack!)** 🔥`;
    else if (isBust) tag = `Value: **${total} (Bust!)** 💥`;
    return `${cardsStr} (${tag})`;
}
...
export const blackjackCommand: Command = {
    ...
    async execute(interaction) {
        ...
        const deductRes = await CoinTransactionLock.deductWager(
            userId,
            amount,
            'gambling_bj',
            `bj:${userId}:${Date.now()}`,
            `Blackjack wager of ${amount} COINS`
        );
        ...
        const finishGame = async (reason: 'bust' | 'dealer', extraDouble = false) => {
            if (finished) return;
            finished = true;
            collector.stop();
            ...
            if (payout > 0) {
                await CoinTransactionLock.grantCoins(
                    userId,
                    payout,
                    isTie ? 'gambling_bj_push' : 'gambling_bj_win',
                    `bj_pay:${userId}:${Date.now()}`,
                    isTie ? 'Blackjack push refund' : `Blackjack win of ${payout} COINS`
                );
            }
        ...
        collector.on('collect', async (btn) => {
            ...
            if (btn.customId.startsWith('bj_double_')) {
                // Deduct second wager
                const doubleDeduct = await CoinTransactionLock.deductWager(
                    userId,
                    amount,
                    'gambling_bj_double',
                    `bj_dbl:${userId}:${Date.now()}`,
                    `Blackjack Double Down additional wager of ${amount} COINS`
                );
