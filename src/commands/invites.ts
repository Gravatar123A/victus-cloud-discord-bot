...
export const invitesCommand: Command = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (subcommand === 'check') {
            ...
            const targetUser: User = interaction.options.getUser('user') || requester;
            const stats = await inviteService.getUserStats(guild, targetUser.id);
            const { container, actionRows } = InviteEmbeds.buildUserInvitesCard(
                stats,
                guild,
                targetUser,
                requester.id
    ...
    async handleButton(interaction: ButtonInteraction) {
        ...
        if (!customId.startsWith('inv:')) return;
