...
export const unscrambleCommand: Command = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        let prefixWordArg: string | null = null;
        let prefixStringArg: string | null = null;
        let prefixTargetChannel: GuildTextBasedChannel | null = null;
        let prefixRoleArg: Role | null = null;
        let prefixNumberArg: number | null = null;
        ...
        if (!subcommand && rawMessage?.content) {
            ...
            if (parts.length > 0) {
                ...
                if (first === 'auto') {
                    subcommand = 'auto';
                    if (parts[1]) prefixStringArg = parts[1];
                    ...
                    subcommand = 'role';
                    const roleMention = parts.find((p: string) => /<@&(\d+)>/.test(p));
                    if (roleMention) {
                        const m = roleMention.match(/\d+/);
                        if (m) prefixRoleArg = interaction.guild.roles.cache.get(m[0]) ?? null;
                    }
        ...
        if (subcommand === 'start') {
            ...
            const result = await unscrambleService.startGame(
                interaction.guild,
                targetChannel,
                interaction.user.id,
                customWord,
        ...
        if (subcommand === 'role') {
            ...
            const targetRole = (interaction.options.getRole?.('role') as Role) ?? prefixRoleArg;
        ...
        if (subcommand === 'status') {
            ...
            if (game) {
                ...
                embed.addFields(
                    { name: '🟢 Active Round', value: `Running in <#${game.channelId}>`, inline: true },
                    { name: '🧩 Scrambled', value: `\`${spaced}\``, inline: true },
                    { name: '🏷️ Category', value: `\`${game.category}\``, inline: true },
                    { name: '🎯 Total Guesses', value: `**${game.guessCount}**`, inline: true },
                    { name: '👥 Participants', value: `**${game.participants.size}**`, inline: true },
                    { name: '💡 Clues Given', value: `**${game.hintsGiven}**`, inline: true }
                );
            } else {
                embed.addFields({
                    name: '⚪ Current Round',
        ...
        if (subcommand === 'leaderboard') {
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(
                    `Top word champions with the most Unscramble victories in **${interaction.guild.name}**!\n\n` +
                    (topWinners.length > 0
                        ? topWinners
                              .map((p, idx) => {
