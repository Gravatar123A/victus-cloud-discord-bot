 ...
 */
async function checkIsStaff(interaction: ButtonInteraction | ModalSubmitInteraction): Promise<boolean> {
    if (!interaction.guildId) return false;

    let member = interaction.member;
    if (!member || !('permissions' in member) || !('roles' in member)) {
        member = (await interaction.guild?.members.fetch(interaction.user.id).catch(() => null)) as any;
    }
...
export const resourceApplyCommand: Command = {
    ...
    async handleSelectMenu(interaction) {
            ...
            await interaction.update({
                ...
                components: [
                    ComponentsV2.cleanContainer(
                        ComponentsV2.Accents.warning,
                        'Already Approved',
                        'This resource listing has already been approved and rewarded by staff.',
                        'REWARD GRANTED'
                    ),
                ],
            });
        ...
        const reviewContainer = ComponentsV2.cleanContainer(
            ...
            `### Staff Verification\n` +
            `Review this resource for community quality and validity. Approving this submission will automatically credit **${REWARD_COINS_AMOUNT} COINS** to <@${userId}>'s Victus Cloud account balance.`,
            'STAFF REVIEW PANEL'
        );
    ...
    async handleButton(interaction) {
            ...
            await interaction.reply({
                components: [
                    ComponentsV2.cleanContainer(
                        ComponentsV2.Accents.danger,
                        'Permission Denied',
                        `Only staff members with the <@&${PRIMARY_STAFF_ROLE_ID}> role can review, approve, or decline resource submissions.`,
                        'ACCESS RESTRICTED'
                    ),
                ],
                flags: MessageFlags.Ephemeral | ComponentsV2.IS_COMPONENTS_V2,
        ...
        if (action === 'victus_res_staff_approve') {
            ...
            const resultContainer = ComponentsV2.cleanContainer(
                ...
                `The reward has been automatically deposited into <@${applicantUserId}>'s Victus Cloud account balance.`,
                'APPROVAL CONFIRMED'
            );
            ...
            try {
                ...
                await applicant?.send(
                    `🎉 **Congratulations!** Your resource submission for **${listing?.title || 'your resource'}** was officially approved by staff member <@${interaction.user.id}>!\n\n` +
        ...
        if (action === 'victus_res_staff_reject') {
            ...
            const resultContainer = ComponentsV2.cleanContainer(
                ...
                `› **Creator:** <@${applicantUserId}>\n` +
                `› **Resource:** ${listing ? listing.title : 'Resource Submission'}\n` +
                `› **Reason:** Does not meet current community resource guidelines.`,
                'SUBMISSION DECLINED'
            );
            ...
            try {
                const applicant = await interaction.client.users.fetch(applicantUserId).catch(() => null);
                await applicant?.send(
                    `⚠️ Your resource submission application for **${listing?.title || 'your resource'}** was reviewed by Victus staff and declined.\n\n` +
    ...
    async handleModal(interaction) {
        if (!interaction.customId.startsWith('victus_res_modal_reject:')) return;
        if (!interaction.guildId) return;
            ...
            await interaction.reply({
                components: [
                    ComponentsV2.cleanContainer(
                        ComponentsV2.Accents.danger,
                        'Permission Denied',
                        'Only authorized staff members can decline resource submissions.',
                        'ACCESS RESTRICTED'
                    ),
                ],
                flags: MessageFlags.Ephemeral | ComponentsV2.IS_COMPONENTS_V2,
        ...
        const resultContainer = ComponentsV2.cleanContainer(
            ...
            `› **Resource:** ${listing ? listing.title : 'Resource Submission'}\n` +
            `› **Reason:** ${reason}\n\n` +
            `The applicant has been notified via direct message.`,
            'SUBMISSION DECLINED'
        );
        ...
        try {
            const applicant = await interaction.client.users.fetch(applicantUserId).catch(() => null);
            await applicant?.send(
                `⚠️ Your resource reward application for **${listing?.title || 'your submission'}** was reviewed by staff and declined.\n\n` +
                `› **Reason:** ${reason}\n\n` +
