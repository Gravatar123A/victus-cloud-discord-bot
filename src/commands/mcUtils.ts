...
function renderGaugeBar(percent: number, size = 12): string {
...
export const mcSkinCommand: Command = {
...
export const mcStatusCommand: Command = {
    data: new SlashCommandBuilder()
        .setName('mc-status')
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const container = ComponentsV2.baseContainer(
                isOnline ? ComponentsV2.Accents.success : ComponentsV2.Accents.danger
            );
    ...
    async execute(interaction) {
        ...
        try {
            ...
            c.addTextDisplayComponents(
                ComponentsV2.text(
                    `# 🛡️ Whitelist Command Dispatched\n\n` +
            ...
            await interaction.editReply({
                content: `❌ Whitelist command failed: ${(err as Error).message}`,
            });
        }
    },
};
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((sub) =>
            sub
                ...
                .addStringOption((opt) =>
                    opt.setName('identifier').setDescription('Server identifier').setRequired(true)
                )
        )
        .addSubcommand((sub) =>
            sub
    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (sub === 'status') {
                ...
                const c = ComponentsV2.baseContainer(
                    state === 'running' ? ComponentsV2.Accents.success : ComponentsV2.Accents.warning
                );
            ...
            await interaction.editReply({ content: `❌ Operation failed: ${(err as Error).message}` });
        }
    },
};
