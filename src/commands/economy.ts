...
async function syncDelta(input: {
    ...
    if (synced === null) logger.warn(`Paymenter COINS sync pending for economy user ${input.userId} (${input.source})`);
    return synced;
}
...
async function buildView(view: string, discordId: string, page = 0): Promise<ContainerBuilder> {
    ...
    const coins = bal.found ? Number(bal.coins) : Number((profile as any).total_cp ?? 0);
