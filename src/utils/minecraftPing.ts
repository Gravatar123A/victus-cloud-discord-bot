 ...
 */
export function parseAddress(addr?: string | null, defaultPort = 25565): { host: string; port: number } | null {
 ...
 */
export function pingMinecraft(host: string, port: number, timeoutMs = 2500): Promise<MinecraftPingResult | null> {
    return new Promise((resolve) => {
        ...
        const finish = (result: MinecraftPingResult | null) => {
            if (!settled) {
                settled = true;
                socket.destroy();
                resolve(result);
 ...
 */
export async function pingServerWithFallback(
    server: { directAddress?: string | null; connectHostname?: string | null; ip?: string },
    timeoutMs = 2500
): Promise<MinecraftPingResult | null> {
