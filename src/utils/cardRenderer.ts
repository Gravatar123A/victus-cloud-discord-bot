...
interface BattlePassCardOptions {
    guildName: string;
...
/**
 * Safely wraps canvas operations in try/catch and dynamic import so that
 * missing platform binaries never crash the Discord bot.
 */
export async function generateLevelCardAttachment(opts: LevelCardOptions): Promise<AttachmentBuilder | null> {
    try {
        ...
        // 2. Ambient background decorative glows
        ...
        // 4. Circular Avatar with glowing ring
...
/**
 * Renders Server Battle Pass celebration card.
 */
export async function generateBattlePassCardAttachment(opts: BattlePassCardOptions): Promise<AttachmentBuilder | null> {
    try {
        ...
        // 4. Milestone Rewards Cards (2 Columns)
        const rewBoxY = 275;
